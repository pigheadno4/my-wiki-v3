...
1. Write the raw file in `raw/` with the pasted content word-for-word (download any CDN image URLs to `raw/assets/` automatically).
...
1. Create or update a source summary page in `wiki/sources/`:
...
2. Create or update relevant company pages in `wiki/companies/`.
3. Create or update relevant concept pages in `wiki/concepts/`.
4. Create or update relevant comparison pages in `wiki/comparisons/` if the source **substantively compares** two or more companies (e.g., pricing differences, feature comparisons, migration guides). A source that merely mentions multiple companies in passing does not warrant a comparison page.
5. Check for contradictions with existing wiki content — flag them.
...
**Example walkthrough** — User asks: _"What are Stripe's chargeback fees compared to Adyen's?"_
...
2. **Verify against raw files** (raw files are NEVER modified — they are immutable source documents):
   ...
   **c. Missing `raw_files:` frontmatter** — Find source pages that lack the `raw_files:` field.
