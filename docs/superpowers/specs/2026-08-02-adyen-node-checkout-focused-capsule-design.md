...
- client configuration, environment selection, authentication, HTTP behavior,
  errors, security, webhook validation, and notification handling; and
...
- `src/index.ts`, `src/client.ts`, `src/config.ts`, `src/service.ts`,
  `src/webhooks.ts`, and `src/typings/index.ts`;
...
At `v32.0.0`, the recursive source roots select 530 files and approximately
2.25 MB before standard context. Initial hard budgets are:
...
When a query needs detailed evidence from an inventory-only domain:
...
- Inventory-only model omissions are expected policy boundaries. A missing
  Checkout root or unclassified retained Checkout change is a blocking gap.
...
- Checkout, Payment, and Recurring generated model roots are retained for deep
  coverage.
