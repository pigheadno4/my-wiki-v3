...
This collected Braintree Control Panel article documents activation and configuration of gateway email receipts for transactions and refunds successfully submitted for settlement. It also covers manual receipt generation from the Control Panel Transaction Details page and separately explains merchant-built custom receipts, while explicitly routing subscription failed-payment alerts to separate notification documentation; the collected snapshot does not establish current availability.
