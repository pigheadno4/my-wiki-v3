...
- Credit-card validations run by default, but verification does not. Account-enabled AVS/CVV checks run during payment-method update and can be skipped with `verifyCard: false`; manual verification uses `verifyCard: true`. When Premium Fraud Management Tools are used, Braintree strongly recommends supplying `device_data` whenever a card is verified. When `verifyCard: true` is used specifically to verify AVS information for an existing payment method, the page says that update creates a transaction; if processing options reject transactions without CVV, the request must include CVV or that rule must be disabled.
...
- AVS-verification update transaction and reject-without-CVV action: `### Updating with a nonce and additional parameters`, line 223.
- Nonce plus additional parameters, duplicate-field warning, customer-association restriction, and raw-data precedence: `### Updating with a nonce and additional parameters` through `### Payment method nonces vs. raw card data`, lines 181-239.
