...
> The pinned page preserves a fetched minimum-version table, cipher-suite lists, and dated certificate records. Verify current support and security requirements with current Braintree authority before changing a live integration. Do not treat a shorter server-SDK timeout as proof that a transaction failed, and do not retry solely from that exception without checking gateway state.
