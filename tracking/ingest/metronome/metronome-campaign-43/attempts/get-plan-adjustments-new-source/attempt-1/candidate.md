...
- Each `data` item is a `PriceAdjustment` associated with a charge and a starting billing period, with one or more price-adjustment entries. Consult the exact schema for adjustment and charge categories, values, quantity, and tier detail rather than treating this summary as a complete response contract.
