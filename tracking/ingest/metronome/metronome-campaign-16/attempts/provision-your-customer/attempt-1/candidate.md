...
- Shared-pool credits link a subscription to a recurring credit, while individual-seat credits require stable seat IDs carried from metric and product grouping through usage events and the contract's seat configuration.
...
`invoice_placement` defaults to `ON_USAGE_INVOICE`, placing the charge on the usage invoice with the matching billing date. `ON_SCHEDULED_INVOICE` instead appends the charge to an existing scheduled invoice with that billing date or creates a new scheduled invoice when none exists. To route invoices and collect payment through Metronome, the contract must include `billing_provider_configuration`; this is a conditional invoicing requirement, not evidence that the create-contract API always requires that field.
...
On the contract, `quantity_management_mode: "SEAT_BASED"` replaces a simple quantity with `seat_config`. That object supplies assigned `initial_seat_ids`, optional `initial_unassigned_seats_quantity`, and the product's `seat_group_key`. Unassigned seats count toward total subscription quantity but receive no credits until a seat ID is assigned. The illustrated recurring commit references the subscription and sets `allocation: "INDIVIDUAL"`.

## Documentation boundaries

The payloads are worked examples, not complete request or response schemas. The page does not define seat-ID collision handling, reassignment or removal mechanics, concurrent edits, exact proration formulas, failure atomicity, idempotency, error responses, or how individual-seat balances behave after a seat is removed. Dedicated current contract and edit-contract API references remain the implementation authority.

No contradiction was found with the existing canonical Metronome subscription, contract, rate-card, billable-metric, event-ingestion, credit-and-commit, or invoicing material. The source adds detailed mechanics where those pages currently give only overview or conditional-schema summaries.

## Related

- Company: [[metronome]]
- Concepts: [[metronome-subscriptions]], [[metronome-credits-and-commits]]
