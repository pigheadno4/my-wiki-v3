...
The existing create-metrics source summary and shared billable-metrics concept describe `value` as a required SQL output column. This page explicitly documents first-column fallback when a multi-column query has no `value` column, so that wiki wording is too strong and should be corrected. No other direct contradiction with the two adjacent source summaries was found.
