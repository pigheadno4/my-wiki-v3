...
- Within a supplied JSON payload, `customer_id` and `customer_billing_provider_configuration_ids` are required. The customer and every configuration ID are UUID-formatted, but the configuration-ID array has no `minItems` or `uniqueItems` constraint and the example repeats one ID. The enclosing OpenAPI `requestBody` is not marked `required: true`, and the payload has no `additionalProperties: false`; omitted-body, empty-array, duplicate-ID, and unknown-field runtime behavior are not established.
...
- Multiple configurations for the same customer can be submitted together, and any validation failure for one prevents the entire operation from succeeding. That statement does not establish validation order, whether IDs must all belong to `customer_id`, concurrency ordering, rollback after execution begins, or recovery from a partial or ambiguous failure.
...
The operation lists generic HTTP `400` and `500` error bodies with required string `message`, but it does not map invalid customer/configuration identity, already-archived state, active-contract conflicts, authentication, authorization, rate limits, downstream failure, or partial effects to specific outcomes. A successful response proves only the documented Metronome archive result, not provider cancellation, invoice delivery or cancellation, payment, refund, tax, settlement, accounting, webhook completion, or reconciliation.

The separate API-wide [[source-metronome-api-reference-idempotency|`Idempotency-Key` authority]] applies to this POST. Metronome persists a provided-key result only after execution begins, meaning validation passed and no pre-execution concurrent-request conflict prevented execution. Identical same-key parameters then replay that original result, changed parameters return HTTP `409`, retention is at least 24 hours, and a persisted result can be HTTP `500`. Validation failures and pre-execution conflicts are not established cached results. The endpoint adds no no-key, another-key, expired-key, repeated-archive, concurrent archive or provider-change, fresh-state verification, rollback, or ambiguous-failure recovery contract; a same-key replay is not fresh proof of current contract, invoice, threshold, metering, provider, or reconciliation state.
...
| Supplied-payload required properties, UUID formats, configuration-ID array, repeated-ID example, absent request-body required marker, and unspecified closed-object behavior | [[raw/metronome/api-reference/customers/archive-billing-provider-configurations-for-a-customer-2026-07-13|complete archive-operation raw]] |
...
- Invoice and retry authorities: [[source-metronome-guides-implement-metronome-core-concepts-how-invoicing-works]], [[source-metronome-api-reference-idempotency]]
