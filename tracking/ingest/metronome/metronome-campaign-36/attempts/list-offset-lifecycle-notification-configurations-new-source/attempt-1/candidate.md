...
- HTTP `200` requires top-level `data`; optional nullable sibling `cursor` is the only response pagination field. The endpoint does not define ordering, total count, cursor lifetime, completion semantics when `cursor` is omitted, snapshot consistency, duplicate-or-skip behavior during configuration changes, as-of time, freshness, or read-after-mutation visibility. The separate API-wide pagination page documents query `next_page`, not this endpoint's body and response `cursor`, so its repeat-until-null convention must not be substituted without endpoint-specific authority.
...
Use the exact raw snapshot for the production server and bearer declaration; `POST /v2/notifications/offset/list` identity and operation ID; user-created lifecycle-relative scope and 400-result request maximum; optional request body; full `limit`, body `cursor`, and three-value archive-filter schema and example; required top-level `data`, optional nullable sibling response `cursor`, and response example; every required configuration identity, environment, creation, and archive field; the required policy fields and positive and negative ISO-8601 offset examples; the absent closed-object declaration, operation-level error catalog, limit range and default, ordering, totals, cursor completion and lifetime, snapshot, freshness, generation, delivery, and reconciliation guarantees. Use the dedicated pagination source only to recognize that its query `next_page` convention differs from this endpoint, the idempotency source for API-wide POST replay, and dedicated notification and webhook authorities for event catalogs, configuration mutations, scheduling behavior, payloads, delivery, retry, deduplication, and verification.
