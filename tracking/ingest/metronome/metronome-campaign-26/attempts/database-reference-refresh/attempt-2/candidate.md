...
- `contracts_commits` documents `cost_basis` directly as the ratio of amount paid for a commit to credit granted. `contracts_balances` also carries `cost_basis`, but that broader table includes postpaid, prepaid, and credit rows while the field description remains commit-specific. The reference does not document whether the field applies to credit rows, what its denominator means for those rows, denominator-zero behavior, or any accounting treatment.
...
- 2026-08-28: Refreshed from the new collection snapshot; preserved prior raw history, updated the invoice-breakdown filter contact route, added schema-change navigation for the commit/balance cost-basis ambiguity, prepaid-threshold commit fields, and alert customer scope, and removed any implication that transition types form a closed two-value set.
