...
- The separate API-wide [[source-metronome-api-reference-idempotency|`Idempotency-Key` authority]] applies when a key is provided and the request begins executing after passing validation and not conflicting with another concurrent request; only then is the result persisted for identical same-key replay. Changed parameters return HTTP `409`, retention is at least 24 hours, and a persisted result can be HTTP `500`. The authority does not say validation failures or pre-execution concurrent-request conflicts are cached, so same-key replay is not always available. This endpoint adds no no-key, another- or expired-key, concurrent-mutation ordering, lost-update-protection, read-after-write-visibility, Salesforce-propagation, partial- or ambiguous-failure, rollback, or recovery guarantee. A replayed result is not a fresh customer-configuration read; after a cached or ambiguous failure, investigate current state, including the separate customer-read surface as evidence only, without treating that read as a consistency guarantee or assuming a changed key is safe.
...
- **Retry and adjacent authority:** use the linked idempotency source for the execution-admission condition on persisted same-key replay, changed-parameter conflict, retention, cached HTTP `500`, and investigation guidance. Validation failures and pre-execution concurrent-request conflicts are not documented as cached. Use customer get/list sources only as current-state evidence routes, not consistency guarantees; use the Salesforce integration source for outbound sync and the billing-provider-configuration source for invoice-destination records and contract selection.
