...
The operation is `POST /v1/listConfiguredBillingProviders` with document-level bearer authentication. Its optional JSON object accepts only a documented `next_page` property: a nullable, UUID-formatted cursor. The page does not define page size, ordering, initial-page cursor conventions beyond omission or nullability, cursor expiry, or invalid-cursor behavior.
