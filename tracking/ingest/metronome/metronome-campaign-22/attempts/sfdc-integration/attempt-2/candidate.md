...
Salesforce receives a distinct customer-ingest-alias object with a Metronome alias ID, a lookup to the associated customer, the alias value, and the Metronome environment. This CRM replica does not establish alias uniqueness, current-active status, reassignment, deletion, ordering, event-matching freshness, or synchronization atomicity. Dedicated event-ingestion authorities continue to own ingest-alias event matching, provisioning limits, persistence, and idempotency.

Salesforce also receives a credit-type object with ID, name, and Metronome environment; the invoice and invoice-line objects each reference a credit type. The combined commit-or-credit object has no documented credit-type lookup even though its total-amount description uses dollar wording. This Salesforce schema therefore does not establish denomination, including USD-cent, non-USD, custom-pricing-unit, conversion, precision, or rounding semantics; those remain outside this guide.
...
- Concepts: [[metronome-integrations]], [[metronome-reporting-and-analytics]], [[metronome-customers-and-contracts]], [[metronome-event-ingestion]], [[metronome-credits-and-commits]], [[metronome-currencies-and-custom-pricing-units]], [[metronome-invoicing]], [[metronome-products-and-rate-cards]]
