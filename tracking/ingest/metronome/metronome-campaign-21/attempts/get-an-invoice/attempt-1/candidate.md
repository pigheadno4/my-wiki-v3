...
The page defines no rate limit, timeout, retry, cache, ordering, read-after-write, snapshot-consistency, historical-visibility, archive, or authorization-scope contract. Because this is GET, the separate API-wide POST idempotency authority does not establish endpoint-specific read guarantees. No cross-page contradiction was found when this Contracts invoice schema is kept distinct from the legacy shared Plans surface; the internal list-price, amount-due, invoice-type, and billable-status documentation gaps above remain unresolved.
