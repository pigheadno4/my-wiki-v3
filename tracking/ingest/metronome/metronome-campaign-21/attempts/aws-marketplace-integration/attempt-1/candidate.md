...
An organization must first be an approved AWS Marketplace seller. The listing can use **Contract with Consumption** or **Usage-based pricing** with **Units** as the dimension unit type; the chosen model must also be represented when the Marketplace customer is configured in Metronome. Contract-with-consumption listings use contract dimensions and **Single dimension per contract**, while usage-based listings omit those contract-only steps.

The integration imposes exact marketplace dimension values:

- Create one usage dimension whose API identifier is `usage_fee`.
- Price every contract dimension at `$0`.
- Price `usage_fee` at `$0.01` per unit.
- Leave **Activate pay-as-you-go pricing for additional usage** unchecked on contract dimensions.
...
- Related sources: [[source-metronome-api-reference-settings-set-up-account-level-billing-provider]], [[source-metronome-api-reference-customers-create-a-customer]], [[source-metronome-guides-customers-billing-manage-customers-schedule-billing-provider-change]], [[source-metronome-guides-invoices-overview]]
