...
- HTTP `200` requires both an AuditLog array under `data` and a nullable string `next_page`. The endpoint describes saving each returned cursor for later polling and says an empty array is a temporary absence of new logs rather than proof that future polls will remain empty.
...
- [[raw/metronome/api-reference/security/get-audit-logs-2026-07-13|2026-07-13 snapshot - account-wide audit-log retrieval, filters, continuous cursor polling, attribution schema, and completeness boundaries]]