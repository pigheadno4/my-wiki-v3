...
> [!warning] Billing-configuration archival
> Archiving a billing configuration that is attached to an active contract immediately archives it on that contract and stops billing to the associated destination. The guide says a new billing-provider configuration cannot then be provisioned on that contract.
