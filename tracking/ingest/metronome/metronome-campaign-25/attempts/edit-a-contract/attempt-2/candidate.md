...
- This endpoint snapshot exposes optional `uniqueness_key` with a 1-128 character schema and wording that reuse prevents a duplicate record and fails with HTTP `409`, although the operation response map omits `409`. The same-date [[source-metronome-api-reference-idempotency|API-wide idempotency authority]] labels contract-edit uniqueness-key support as coming soon, so schema exposure here does not resolve runtime enablement. This unresolved authority conflict is distinct from API-wide POST `Idempotency-Key` result replay. Neither page defines the two keys' interaction, uniqueness-key scope or release, no-key or expired-key retries, concurrent ordering, or recovery after cached or ambiguous failures.
...
- Company: [[metronome]]
- Primary concepts: [[metronome-customers-and-contracts]], [[metronome-invoicing]], [[metronome-credits-and-commits]], [[metronome-spend-threshold-billing]], [[metronome-api-idempotency]], [[metronome-integrations]]
...
- [[raw/metronome/api-reference/contracts/edit-a-contract-2026-07-13|2026-07-13 snapshot - contract-edit mutation, required identifiers, mutable term families, invoice and recurring-grant lifecycle effects, threshold charge initiation, duplicate prevention, provider limits, response schema, and errors]]
