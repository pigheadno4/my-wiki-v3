...
The test request uses a JSON body containing `"name": "Example-Customer"`. The displayed response contains `data.id`, `data.external_id`, and `data.name`; because the page labels the request as a test and supplies no schema, these fields and values should not be treated as a complete API specification.
...
- The claim that the OpenAPI specification is always up to date does not establish a versioning or compatibility policy. A team that needs reproducible tests must retain its own approved specification version or import record.
