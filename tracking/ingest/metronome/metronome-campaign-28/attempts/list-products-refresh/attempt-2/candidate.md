...
- Optional top-level `ProductListItem.custom_fields` sits beside `initial`, `current`, and `updates`; it is an arbitrary-key string map annotated for `contract_product`, not a field on `ProductListItemState` or `ProductListItemUpdate`. Separately, state and update objects can expose metric, composite, quantity-conversion, rounding, tag, group-key, and configuration-dependent NetSuite fields. Quantity conversion and rounding are explicitly usage-only; pricing and presentation group keys are usage-only and require their combined value superset as one compound billable-metric group key. These response surfaces do not prove configured-field completeness, create/edit acceptance, propagation, product-type applicability beyond their own annotations, or downstream billing and reconciliation behavior.
...
- The optional top-level product custom-field map does not establish configured-key validation, completeness, mutation, persistence, visibility, or invoice-line propagation. The separate NetSuite item identifiers belong to state and update schemas and are configuration-dependent; their presence does not establish mapping validity, synchronization, invoice delivery, accounting, settlement, or reconciliation.
