...
- At immediate parent `SchedulePointInTimeInput`, the description says to provide either `schedule_items` or `recurring_schedule`, but the object itself has no OpenAPI required list. A point-in-time item requires only `timestamp`; it can provide numeric `amount` or both `unit_price` and `quantity`, with their product determining amount, while sending `amount` makes unit price equal amount and quantity equal 1. The recurring object requires `starting_at`, `ending_before`, `frequency`, and `amount_distribution` and exposes the same amount-or-pair forms. Its optional `credit_type_id` defaults to USD cents. The page gives no precision, rounding, sign, bounds, tax, collection, or reconciliation contract.
...
| Invoice schedule | Point-in-time and recurring forms, nested required lists, amount versus unit-price-and-quantity rules, frequency and distribution enums, and `do_not_invoice` default and scope |
