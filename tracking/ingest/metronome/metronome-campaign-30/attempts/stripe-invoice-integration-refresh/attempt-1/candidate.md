...
- For `charge_automatically`, Metronome sends the finalized invoice immediately, but Stripe documents an up-to-one-hour wait before its payment attempt and a 72-hour fallback when `invoice.created` delivery fails. Decimal quantities, more than 250 line items, Stripe's documented maximum charge, and credit-memo absence can change or limit the Stripe representation even though the detailed Metronome invoice remains available.
