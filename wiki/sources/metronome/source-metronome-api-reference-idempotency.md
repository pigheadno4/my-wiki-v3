...
> [!warning] Retry guidance requires reconciliation
> The separate status-code page advises verifying whether a resource was partially created after a `5XX` response and then retrying with a different idempotency key. This idempotency page says HTTP 500 results are cached and recommends investigating state before deciding whether to retry or resolve manually. Neither page defines a universal safe sequence for every partially completed operation.

## Related

- Company: [[metronome]]
- Concepts: [[metronome-api-idempotency]], [[metronome-event-ingestion]], [[metronome-customers-and-contracts]]
- Related sources: [[source-metronome-api-reference-contracts-amend-a-contract]], [[source-metronome-api-reference-customers-create-a-customer]], [[source-metronome-api-reference-usage-ingest-events]], [[source-metronome-api-reference-contracts-create-a-contract]], [[source-metronome-api-reference-status-codes]]
