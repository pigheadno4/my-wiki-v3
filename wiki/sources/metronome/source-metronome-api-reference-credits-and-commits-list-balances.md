...
> The contract-archive authority establishes that archiving a contract archives its associated commits and credits. This list schema does not reconcile that lifecycle fact with its asymmetric visibility surface: it remains unknown whether `include_archived` returns archived commits, how Credit exposes archive status without `archived_at`, how the repeated-credit wording partitions archived credits versus credits from archived contracts, or what omission of `include_archived` does.
