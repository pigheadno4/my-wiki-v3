...
- Separate API-wide [[metronome-api-idempotency|POST idempotency guidance]] applies: the same key with identical parameters replays the original result, changed parameters return `409`, retention is at least 24 hours, and a cached result can be HTTP `500`. A keyed replay is not a fresh search or proof of later ingest or matching state.
...
## API-wide idempotency and replay-freshness boundary

The separate [[source-metronome-api-reference-idempotency|Metronome API idempotency reference]] states that `Idempotency-Key` applies to all POST endpoints. Reusing the same key with identical parameters returns the original result, changing parameters with the same key returns HTTP `409 Conflict`, keys are retained for at least 24 hours, and a cached result can be HTTP `500`.

For this search operation, replaying the original keyed response is a request-result guarantee, not evidence that the response reflects events ingested or customer and metric matching changed after the first request. This endpoint page does not define response behavior without a key, with a different or expired key, under concurrent calls, after a desired snapshot refresh, or during search-specific recovery from a cached error.

## API boundaries and unknowns

The operation's own response map lists no `400`, `401`, `403`, `404`, `409`, `429`, or `5xx` contract even though the endpoint is bearer secured and described as heavily rate limited. The endpoint page does not define how absent, expired, malformed, or duplicate requested IDs are represented; whether one result is returned per input ID; whether extra events can appear; result ordering; pagination or truncation; maximum response size; partial results; timeouts; response freshness without a reused idempotency key; rate-limit headers; endpoint-specific retry guidance; or authorization scope beyond the global bearer scheme.
...
- Company: [[metronome]]
- Concepts: [[metronome-event-ingestion]], [[metronome-billable-metrics]], [[metronome-api-idempotency]]
- Related sources: [[source-metronome-guides-events-high-volume-ingestion]], [[source-metronome-guides-implement-metronome-core-concepts-create-billable-metrics]], [[source-metronome-guides-implement-metronome-production-checklist]], [[source-metronome-api-reference-usage-ingest-events]], [[source-metronome-api-reference-idempotency]]
