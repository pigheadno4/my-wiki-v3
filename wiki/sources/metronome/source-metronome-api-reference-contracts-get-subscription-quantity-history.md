...
Send a bearer-authenticated `POST` request to `/v1/contracts/getSubscriptionQuantityHistory`. The OpenAPI operation defines `requestBody` content but does not mark `requestBody` itself as required, so this page does not document omitted-body behavior. Within the JSON object schema, `customer_id`, `contract_id`, and `subscription_id` are required UUID strings. The page does not document query parameters, pagination inputs, or a request-side time range.
