...
- The endpoint excludes archived products by default. The JSON body may be omitted because `requestBody` is not marked required, and an empty object conforms because the payload schema requires no property. When supplied, the payload must be a non-null object and `archive_filter` must be a non-null string accepting `ARCHIVED`, `NOT_ARCHIVED`, or `ALL`.
...
| JSON payload | Body optional; if supplied, optional non-null string `archive_filter`: `ARCHIVED`, `NOT_ARCHIVED`, or `ALL`; omission defaults to not archived |
...
The operation's `requestBody` contains an `application/json` schema reference and an example, but has no `required: true`, so body omission is allowed by the documented operation schema. `ListProductsPayload` is an OpenAPI 3.0.1 `type: object` with no required-property list, so an empty object conforms; its only documented property is `archive_filter`, a `type: string` enum. Neither the payload object nor `archive_filter` sets `nullable: true`, so an explicitly supplied null body or null property is outside the documented schema. The endpoint does not document the runtime error status, envelope, or handling for either invalid null. Because `additionalProperties` is unspecified on `ListProductsPayload`, preserve the unknown-property boundary rather than claiming that extra properties are accepted or rejected.
