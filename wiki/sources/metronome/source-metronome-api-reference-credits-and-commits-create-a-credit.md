...
- Contract scope can be limited with `applicable_contract_ids`; the prose says an empty value is cross-contract, while the schema says omission applies to all contracts. The direct-selector descriptions say omitting both product IDs and tags makes the credit apply to all products, while the same payload permits `specifiers` only when those direct selectors are absent and says at least one specifier condition must match. The page does not reconcile those statements into one selector algorithm.
...
This required-property list must not be presented as proof that a completely omitted body is rejected: the wrapper does not declare that requirement. The payload schema also does not set `additionalProperties`, so it does not say whether unknown top-level fields are accepted, ignored, or rejected. Under OpenAPI 3.0.1, the typed properties shown without `nullable: true` are non-null at the schema boundary; this includes required `customer_id`, `priority`, `product_id`, and `access_schedule`, as well as optional typed fields when present. The page does not define endpoint-specific runtime error mapping for invalid nulls, numeric bounds for priority, authorization scope beyond bearer authentication, or other field-level failure mappings.
...
The required `product_id` identifies the credit's product even when eligible usage is unrestricted. The `applicable_product_ids` and `applicable_product_tags` descriptions each say that when both direct selectors are absent, the credit applies to all products. However, the same payload permits `specifiers` only when those direct selectors are absent and says at least one specifier condition must match. The page does not reconcile whether the presence of `specifiers` overrides the direct-selector descriptions' all-products wording, so these statements must remain an unresolved applicability ambiguity rather than a synthesized selector algorithm. It also does not state whether the two direct selector fields may be combined with each other or how empty arrays differ from omission.

Alternatively, `specifiers` filter eligible usage. Usage must satisfy at least one specifier. A specifier can select one product UUID, require all listed product tags, and match arbitrary string-valued pricing-group or presentation-group maps. `specifiers` cannot be combined with either direct product selector. The feature-gated `exclude` array makes a specifier inapplicable when usage matches its inclusion criteria and any exclusion entry; within one `ExcludeSpecifier`, `product_tags` matches products carrying all tags listed in that entry. This does not require usage to match all tags across every exclusion-array entry. No field inside a specifier or exclusion object is marked required, and the page does not define empty-specifier behavior, selector evaluation against missing dimensions, case sensitivity, duplicate entries, atomic validation, or unknown fields on these enclosing objects.
...
`custom_fields` is labeled for the `contract_credit` entity and references a typed object whose arbitrary property values are typed strings. Neither the object schema nor its string-valued `additionalProperties` sets `nullable: true`, so a supplied `custom_fields` value and each supplied property value are non-null at the OpenAPI 3.0.1 schema boundary. This endpoint does not define allowed keys, configured-key validation, uniqueness, limits, endpoint-specific runtime error mapping for invalid nulls, deletion-by-null semantics, overwrite behavior, persistence, deletion, or propagation; those broader custom-field behaviors remain owned by the dedicated custom-fields authority. NetSuite sales-order and Salesforce opportunity IDs are strings whose availability depends on client configuration. `rate_type` accepts uppercase and lowercase `COMMIT_RATE` and `LIST_RATE`, but the page gives no semantics or precedence for those values.
...
The operation lists `400 Bad request` and generic `404 Not found`; both use an error object requiring only string `message`. It gives no endpoint-specific error codes, examples, or mappings and does not list `401`, `403`, `409`, `429`, or `5xx`. Separately, `uniqueness_key` is a 1-128 character string whose reuse after creating a credit or commit prevents a new record and fails with `409`. The endpoint page does not define key scope, normalization, races, or whether a failed attempt consumes the key. The separate API-wide authority says uniqueness keys last until released but documents release only for Alerts, so it provides no documented customer-credit release path.
