...
Bearer-secured `POST /v2/contracts/commits/archive` deactivates a contract-level or customer-level commit while preserving historical records. Archival is blocked until every finalized usage invoice to which the commit was applied and every finalized commit-payment invoice is voided; the documented correction flow then regenerates the voided usage invoice without the archived commit. The archived commit has a null ledger, zero remaining balance, and a fully deactivated access schedule, and is omitted by default from commit and balance lists unless `include_archived` is used. Operators who only need to reduce a grant are routed to access-schedule editing or a manual balance-ledger entry instead. [[source-metronome-api-reference-credits-and-commits-archive-a-commit]]

The deprecated Plans `POST /v1/credits/createGrant` creates a customer credit grant with required granted and paid amounts, expiry, invoice-visible name, and an unexplained numeric priority within a supplied payload. Optional effective timing and `product_ids` govern when and where it applies; product omission applies it to all products, while supplied array order controls legacy invoice-line application. Do not import that rule as the current Contract balance or invoice-line algorithm: the dedicated prioritization authority first separates rollover, prepaid, and postpaid balance types and then applies source-specific tie-breakers, while eligible invoice lines use product type, start date, unit price, and name. Optional resource `uniqueness_key` rejects reuse with HTTP `409`; the legacy void endpoint can reset that grant key with `release_uniqueness_key: true`, qualifying the API-wide page's Alerts-only release statement. The independently documented current customer-level route is `POST /v1/contracts/customerCredits/create`, and current guidance recommends contract create or edit for most credits, but these authorities establish no one-to-one replacement, identity mapping, field mapping, migration procedure, or removal date for the legacy grant. [[source-metronome-api-reference-credit-grants-create-a-credit-grant]] [[source-metronome-guides-pricing-packaging-apply-credits-and-commits-prioritization-rules]] [[source-metronome-api-reference-credit-grants-void-a-credit-grant]] [[source-metronome-api-reference-credits-and-commits-create-a-credit]] [[source-metronome-api-reference-contracts-create-a-contract]] [[source-metronome-api-reference-contracts-edit-a-contract]]
...
Credits and commits can restrict eligible usage with direct product IDs or tags for simple product or product-family filtering, or with `specifiers` for pricing-group values, presentation-group values, and advanced boolean logic. Direct selectors match when usage matches any listed product ID or tag; within a specifier fields are ANDed, while separate specifier objects are ORed. A pricing- or presentation-group condition cannot match a product missing the corresponding group key, and subscriptions and composite products do not carry those group values. [[source-metronome-guides-pricing-packaging-apply-credits-and-commits-target-credit-and-commits]]
...
Archiving a billing-provider configuration automatically disables credit-based threshold recharge configurations for contracts using that provider. The archive endpoint does not define whether credit-based maps exactly to `prepaid_balance_threshold_configuration`, what happens to an already initiated payment, payment-gate workflow, pending commit, invoice, balance or ledger record, whether disablement is atomic with contract archival, or how operators restore or replace the billing route and safely re-enable recharge. Do not infer cancellation, rollback, provider reconciliation, or duplicate-charge prevention from disablement alone. [[source-metronome-api-reference-customers-archive-billing-provider-configurations-for-a-customer]]
...
## Sources
- [[source-metronome-api-reference-credits-and-commits-list-commits]] — customer prepaid and postpaid commit listing, contract/archive scope, optional ledger and current-balance views, and exact request and response locators
- [[source-metronome-api-reference-credit-grants-update-a-credit-grant]] — deprecated Plans credit-grant editing by grant ID, Contracts migration boundary, editable name and expiration fields, and the `credit_grant_type` schema conflict
- [[source-metronome-api-reference-credits-and-commits-update-the-credit-end-date]] — customer-credit end-date shortening, exclusive drawdown cutoff, no-extension boundary, edit-credit route, and credit-versus-commit wording caution
- [[source-metronome-api-reference-credits-and-commits-get-the-net-balance-of-a-customer]] — combined customer credit-and-commit balance retrieval, OR-between-filter and AND-within-filter composition, denomination and draft-invoice modes, hierarchy exclusion, non-negative segment treatment, and exact request and response locators
