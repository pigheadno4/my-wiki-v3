...
Bearer-authenticated `POST /v1/customers/{customer_id}/setIngestAliases` targets one required UUID customer and fully replaces, rather than merges into, that customer's alias set. A supplied payload requires `ingest_aliases`, with at most 2,000 strings of 1-128 characters each; the enclosing body is not marked required, no closed-object policy is declared, and the array has no `minItems`, so omitted-body, unknown-field, duplicate-element, and runtime empty-array behavior remain undocumented. The endpoint says switching an alias associates all corresponding usage with the new customer. Read with the API-wide alias authority, switching requires removal from the original customer first, including when that customer is archived; the endpoint does not establish atomic cross-customer transfer, concurrent-writer ordering, read-after-write visibility, rollback, or which historical, rated, invoiced, finalized, exported, or downstream records are reassociated. The operation lists HTTP `200` with description `Success` but provides no response content schema or example, and therefore exposes no documented applied-set representation, affected-event list, propagation state, or operation identifier; the endpoint failure catalog also remains undocumented. [[source-metronome-api-reference-customers-create-or-update-customer-ingest-aliases]] [[source-metronome-api-reference-idempotency]]
...
### Package-scoped contract association list

Bearer-authenticated `POST /v1/packages/listContractsOnPackage` returns package-associated projection items whose required fields are `customer_id`, `contract_id`, and `starting_at`; optional `ending_before` and `archived_at` describe the projection's time and archive surfaces. A supplied payload requires `package_id`, but the enclosing request body is not marked required and unknown-property behavior is unspecified. `starting_at` and `covering_date` are mutually exclusive, `include_archived` defaults false, and query `limit` and `next_page` paginate results. This projection is not a contract-detail or current-state authority: it omits package identity, contract terms, pricing, rate-card, invoice, mutation, and migration-result fields, and the page defines no ordering, total count, snapshot consistency, freshness, relationship-validation behavior, or read-after-change guarantee. [[source-metronome-api-reference-contracts-list-contracts-associated-with-a-package]]
...
Bearer-authenticated `GET /v1/customers/{customer_id}/plans` lists one UUID-identified customer's legacy Plan assignments in reverse-chronological order. Each item distinguishes customer-plan `id` from catalog `plan_id`, requires `starting_on`, and may expose `ending_before`; optional `limit` and `next_page` paginate required top-level `data` plus a nullable sibling cursor. For new integrations, the separately authoritative [List customer contracts (v2)](https://docs.metronome.com/api-reference/contracts/list-customer-contracts-v2) route is `POST /v2/contracts/list`: it lists a customer's Contracts chronologically for provisioning, current-agreement, and tier-history queries, supports `starting_at`, mutually exclusive `covering_date`, and `include_archived` filters, and requires a `data` array without documenting the legacy Plan GET's cursor. [[source-metronome-api-reference-contracts-get-a-contract-v2]] covers `POST /v2/contracts/get` for one identified Contract, and [List customer contracts (v1)](https://docs.metronome.com/api-reference/contracts/list-customer-contracts-v1) is itself a legacy `POST /v1/contracts/list` authority. These exact routes and source-scoped differences do not establish shape compatibility or a Plan-to-Contract or assignment-to-contract identity, field, or migration mapping. [[source-metronome-api-reference-plans-list-customer-plans]]
...
Bearer-authenticated `GET /v1/customers/{customer_id}/costs` is a deprecated Plans endpoint for one customer's daily pending costs. Required inclusive and exclusive time bounds and optional cursor pagination return interval records grouped by credit type and optional line-item breakdown. Metronome directs new clients to Contracts but provides no replacement operation, Plan-to-Contract or cost-field mapping, historical-continuity rule, migration procedure, compatibility period, or removal date. The endpoint also supplies no Plan or contract identifier in its response and does not define how the unsupported `UNIQUE`-metric condition maps to Contracts. [[source-metronome-api-reference-invoices-get-customer-costs]]
...
A supplied embeddable-dashboard payload requires one Metronome customer UUID and selects `invoices`, `usage`, or `commits_and_credits`; the endpoint returns a customer-specific URL rather than a cross-customer catalog. It does not define archived-customer eligibility, contract selection outside the invoice-only `contract_id` option, hierarchy aggregation, or behavior when the customer has several contracts. The separate hierarchy guide remains the authority that invoice and commit embeddable dashboards do not work for customers whose contract participates in a hierarchy; this endpoint neither repeats that limit nor states whether `usage` is affected or which error occurs. [[source-metronome-api-reference-customers-get-an-embeddable-customer-dashboard]] [[source-metronome-guides-pricing-packaging-billing-model-guides-model-hierarchical-customer-relationships]]

`POST /v1/contracts/createHistoricalInvoices` accepts an array whose items each require UUID customer, contract, and credit-type identities; inclusive start, exclusive end, issue date; and usage-line items. The operation is contract-scoped but does not establish that the customer owns the contract, that invoice and line-item periods align with the contract or one another, that periods are actually past, or that an imported period cannot overlap an automatically generated or previously imported invoice. Neither `invoices` nor required `usage_line_items` declares `minItems`, and the endpoint does not define batch ordering, all-or-nothing atomicity, partial success, rollback, contract edit-history visibility, or propagation. Deferred automatic generation through `invoice_generation_starting_at` remains a separate contract-creation control rather than a uniqueness or overlap guard for this endpoint. [[source-metronome-api-reference-contracts-create-historical-invoices]] [[source-metronome-api-reference-contracts-create-a-contract]]

### Customer balance views

`POST /v1/contracts/addManualBalanceLedgerEntry` selects a required customer, balance, and segment. Optional `contract_id` selects a contract-level balance; leaving it blank updates a customer-level balance. This establishes targeting, not whether a balance belongs to the supplied customer or contract when identifiers conflict, how customer-level balance is shared across contracts, or how the adjustment propagates to contract reads, hierarchy views, invoices, or exports. [[source-metronome-api-reference-credits-and-commits-add-a-manual-balance-entry]]
...
Bearer-authenticated `POST /v1/contracts/updateEndDate` adds, changes, or removes a contract's exclusive end timestamp. Ending early impacts draft usage statements, truncates terms, and removes upcoming scheduled invoices; moving the date later extends only contract length, not terms, scheduled invoices, or in-advance subscriptions. Optional `allow_ending_before_finalized_invoice` defaults to `true`: finalized invoices remain unchanged when the new end precedes their end timestamps, and Metronome says callers can void and regenerate finalized usage invoices to incorporate the new end date. [[source-metronome-api-reference-contracts-update-the-contract-end-date]]
...
Bearer-authenticated `POST /v1/archiveCustomerBillingProviderConfigurations` archives one customer's selected billing-provider configurations immediately. For an active contract using one, the endpoint says Metronome archives the configuration on the contract and immediately stops metering to downstream systems; a grace-period invoice is not sent to the customer, and associated spend- and credit-based threshold recharge configurations are disabled. This is not the scheduled provider-change operation and does not establish continuity to another destination. The endpoint does not identify affected contracts, define whether downstream metering includes Metronome rating or only provider reporting, or specify replacement, restoration, retention, read-after-write visibility, concurrent schedule-edit ordering, invoice cancellation, downstream cancellation, or reconciliation. The separate provisioning guide adds a beta-scoped statement that the configuration becomes reusable on another customer and cannot be replaced on the affected active contract; the archive endpoint itself promises only uniqueness-key reuse for new configurations, not configuration-ID reassignment. [[source-metronome-api-reference-customers-archive-billing-provider-configurations-for-a-customer]] [[source-metronome-guides-implement-metronome-core-concepts-provision-customer]] [[source-metronome-guides-customers-billing-manage-customers-schedule-billing-provider-change]]
...
## Sources
- [[source-metronome-api-reference-contracts-get-a-contract-v1]] — legacy bearer-authenticated single-contract retrieval by customer and contract UUID, with optional balance and ledger expansion and exact returned-contract schema navigation
- [[source-metronome-api-reference-plans-end-a-customer-plan]] — deprecated customer-Plan end-date mutation, exclusive midnight-UTC cutoff, conditional invoice voiding and Stripe void/delete attempt, and exact target and payload locators
- [[source-metronome-api-reference-contracts-list-customer-contracts-v2]] — chronological v2 customer-contract listing for current-agreement and tier-history retrieval, with exact selection, response, and embedded-collection completeness routes
- [[source-metronome-api-reference-contracts-list-customer-contracts-v1]] - legacy customer-contract history and current-agreement listing, customer and time selectors, optional ledger and balance expansion, and exact returned-contract schema routes
- [[source-metronome-api-reference-plans-add-a-plan-to-a-customer]] - deprecated customer-to-Plan association for a specified date range, with Contracts migration warning and exact customer, Plan, interval, adjustment, and response locators
- [[source-metronome-api-reference-plans-get-the-plan-adjustments-for-a-customer]] - deprecated customer-Plan price-adjustment lookup by customer and customer-plan relationship identity, with exact response-schema navigation and no documented Contracts replacement mapping
...
- [[source-metronome-api-reference-plans-list-plan-charges]] - deprecated Plan-specific charge listing and explicit direction for new clients to use Contracts, without a replacement-operation mapping
...
- [[source-metronome-api-reference-contracts-list-contracts-associated-with-a-package]] - package-scoped customer and contract projections, time and archive filters, cursor placement, and contract-detail, freshness, and migration boundaries


- [[source-metronome-api-reference-credits-and-commits-add-a-manual-balance-entry]] - customer, optional contract, balance, and segment targeting for a manual ledger event, with relationship-validation and propagation boundaries
...
- [[source-metronome-guides-pricing-packaging-apply-credits-and-commits-guarantee-zero-overages]] - per-contract commit and commit-scoped override configuration for customer-specific zero-overage pricing, with merchant-owned access enforcement
