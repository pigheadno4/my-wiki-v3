...
On deprecated Plans `POST /v1/credits/createGrant`, optional `product_ids` limits a grant to selected products; omission applies credits to charges for all products, and supplied array order determines legacy application across invoice line items. Its required numeric `priority` is not explained. Do not import either statement as the current Contract ordering algorithm: the dedicated prioritization authority orders current applicable balances by balance type and then source-specific priority, applicability, cost-basis, and schedule tie-breakers, while eligible invoice lines are independently ordered by product type, earlier start date, higher unit price, and name. The legacy page also leaves empty arrays, invalid or duplicate IDs, later-created products, and interaction with current Contract applicability undefined. [[source-metronome-api-reference-credit-grants-create-a-credit-grant]] [[source-metronome-guides-pricing-packaging-apply-credits-and-commits-prioritization-rules]]


Bearer-authenticated `POST /v1/contract-pricing/products/create` creates a product from schema-required `name` and `type` properties inside a supplied payload; the enclosing request body is not marked required. HTTP `200` requires `data` referencing a generic `Id` with UUID `id`, but does not label that value as created-Product identity. The create request accepts case variants of four guide-documented types plus client-annotated `PROFESSIONAL_SERVICE` and `PRO_SERVICE`; current Get/List responses expose only five uppercase values including `PRO_SERVICE`, while the guide documents four. For composites, create descriptions call both product-ID and tag arrays required despite their absence from the schema required array, while the guide says applicable products can be selected by ID or tag. Preserve these source-scoped enum and selector conflicts without inferring response normalization, professional-service availability, or whether both, either, or at least one selector is enforced. Creation does not itself assign pricing or attach the product to a rate card, contract, or invoice. [[source-metronome-api-reference-products-create-a-product]] [[source-metronome-api-reference-products-get-a-product]] [[source-metronome-api-reference-products-list-products]] [[source-metronome-guides-implement-metronome-core-concepts-create-products-contracts]]

Bearer-authenticated `POST /v1/contract-pricing/products/archive` irreversibly archives one product selected by required payload UUID `product_id`. Current rate cards already associated with the product continue to function, but the archived product is no longer selectable for newly created rates; it remains retrievable in the UI and API and cannot be unarchived. The narrow continuity statement does not define whether existing rates retain a snapshot or live product reference, nor behavior for concurrent or scheduled new-rate creation, aliases, packages, contracts, overrides, late or corrected usage, historical rating, invoice recalculation, exports, reports, or propagation across catalog surfaces. HTTP `200` requires `data.id` and the example repeats the requested UUID, but the generic schema does not label product-resource versus archive-operation identity or expose affected rates, state, or completion time. [[source-metronome-api-reference-products-archive-a-product]] [[source-metronome-api-reference-products-get-a-product]] [[source-metronome-api-reference-products-list-products]]
...
When a streaming billable metric's calculation configuration must change, the Update Metric reference gives a replacement workflow: duplicate the metric, make and save the changes, navigate to the Product associated with the incorrect metric, and schedule that Product to reference the new metric on the appropriate date. This does not establish discovery or migration of every associated Product, archival of the original metric, atomicity, rollback, effective-time inclusivity or timezone, accepted or late-event behavior, cross-transition aggregation, historical invoice or report recalculation, or propagation to rating and downstream systems. [[source-metronome-api-reference-billable-metrics-update-a-billable-metric]]
