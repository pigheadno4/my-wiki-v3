...
`POST /v1/customers/{customer_id}/setIngestAliases` replaces the aliases that usage events may supply in their `customer_id` field. The page says moving an alias to another customer associates all corresponding usage with the new customer and permits several aliases to model child organizations under one Metronome customer. The API-wide authority requires removing an in-use alias from its original customer before assignment elsewhere. Neither source defines when the new match becomes visible; atomicity with concurrent ingestion; event reprocessing; transient unmatched state; or effects on already rated usage, credit ledgers, alerts, draft or finalized invoices, reports, exports, webhooks, CRM replicas, or downstream billing systems. The operation lists HTTP `200` with description `Success` but provides no response content schema or example, and therefore exposes no documented applied-set representation, affected-event list, propagation state, or operation identifier; it does not prove that every usage and financial surface has converged. [[source-metronome-api-reference-customers-create-or-update-customer-ingest-aliases]] [[source-metronome-api-reference-idempotency]]
...
After events are accepted and matched to customers and billable metrics, bearer-authenticated `POST /v1/usage` can read aggregated post-metric usage across multiple customers and metrics by requested time windows. The page defines no read-after-ingest timing, freshness, stable snapshot, or completeness guarantee, so a completed batched traversal does not prove that every submitted event was accepted, deduplicated, matched, or made visible. Raw-event submission, transaction-ID deduplication, matching diagnostics, and ingest recovery remain with their dedicated authorities. [[source-metronome-api-reference-usage-get-batched-usage-data]]
