...
Metronome describes a Product as the basic unit of an invoice line item and says its meaningful name appears on customer invoices. The create operation itself does not assign a price, attach the product to a rate card or contract, prove invoice adoption, or define draft/finalized behavior, delivery, collection, tax, payment, accounting, or reconciliation. Its HTTP `200` generic `data.id` schema also does not explicitly identify the returned UUID as Product-resource identity. [[source-metronome-api-reference-products-create-a-product]]
...
### Native NetSuite billing and revenue-system delivery

A contract can route a finalized invoice to NetSuite as billing provider, after which NetSuite owns distribution, tax calculation, and payment collection. Alternatively, NetSuite can be the revenue system while another provider bills: Metronome sends the invoice after a payment attempt, adds a linked payment object on success, can represent a first-attempt failure as `OPEN`, and can later update it to `PAID` after a successful reattempt. Billing-mode observations live under `external_invoice`, while revenue-mode observations live under `revenue_system_invoices`; failure information and sync notifications support diagnosis, and a corrected item mapping can be followed by a manual resend. These surfaces do not prove external acceptance, customer delivery, payment finality, tax correctness, revenue posting, settlement, or reconciliation.
...
The deprecated Plans `GET /v1/customers/{customer_id}/costs` endpoint returns documented daily pending-cost intervals for one customer, grouped by arbitrary credit-type ID and then optionally by line items. It is not an invoice-object read: the response exposes no invoice, Plan, contract, status, issue, finalization, revision, or downstream-provider identity. Required inclusive `starting_on` and exclusive `ending_before` bound the read; cursor traversal has no documented ordering, snapshot, freshness, or consistency guarantee. The operation is unsupported when the customer's Plan includes a `UNIQUE`-type billable metric and directs new clients to Contracts without naming a replacement route or mapping. Do not treat current invoice List, Get, or Breakdowns endpoints as an inferred substitute, and do not extend pending costs to invoice creation, finalization, delivery, collection, payment, tax, accounting, or reconciliation. [[source-metronome-api-reference-invoices-get-customer-costs]]
...
The dedicated bearer-authenticated `POST /v1/contracts/createHistoricalInvoices` schema requires `invoices` and `preview` only within a supplied payload; the enclosing request body is not marked required. Each historical invoice requires customer, contract, credit-type, service-period, issue-date, and usage-line inputs, while HTTP `200` requires a `data` array of generic invoice objects whose immediate required fields are `id`, `customer_id`, `credit_type`, `line_items`, `status`, `total`, and `type`. The same response schema covers preview and creation, so it does not establish preview persistence, stable IDs, a commit operation, one output per input or input order, the exact created status or type, or whether an existing past-period invoice is replaced. Although the endpoint describes both migrations and correcting past billing periods, it defines no correction-state precondition, overlap or uniqueness control, void/regenerate action, external A/R effect, refund, tax, payment, provider acceptance, or reconciliation. Keep it distinct from draft usage correction, finalized-invoice compensating action, and credit-and-rebill. [[source-metronome-api-reference-contracts-create-historical-invoices]] [[source-metronome-guides-invoices-invoice-optimization-import-existing-invoices]] [[source-metronome-guides-invoices-invoice-optimization-issue-credit-memos]]

### Credit, correction, and re-bill state boundaries

A manual commit-or-credit balance entry can address discrepancy scenarios, but it appends a balance-ledger event rather than correcting the underlying usage or pricing record. The endpoint guidance says most inaccurate-billing corrections can instead be performed upstream through contract editing, rate editing, or another action that recalculates the invoice. A successful manual entry does not establish invoice recalculation or regeneration, finalized-invoice change, refund, payment, tax recomputation, revenue posting, external A/R correction, or reconciliation; those require their own authorities and state checks. [[source-metronome-api-reference-credits-and-commits-add-a-manual-balance-entry]]

`POST /v2/contracts/edit` applies contract changes immediately to draft invoices. Finalized invoices remain unchanged unless voided and regenerated in the UI or API. The default-true `allow_contract_ending_before_finalized_invoice` control can permit a new contract end before existing finalized-invoice end timestamps without changing those invoices; finalized usage invoices must be voided and regenerated to incorporate the end date. Enabling either spend or prepaid-balance thresholds from `false` to `true` causes immediate evaluation, and reaching the usage or balance threshold initiates a charge, but initiation does not prove collection, commit availability, or downstream invoice/provider reconciliation. Mixed-edit atomicity, payment/refund behavior, tax recalculation, external posting, webhook timing, and ambiguous-failure recovery remain undefined. [[source-metronome-api-reference-contracts-edit-a-contract]]
...
Metronome exposes globally bearer-secured `POST /v1/invoices/regenerate` to regenerate a voided invoice. It recalculates from up-to-date rates, available balances, and other fees regardless of billing period, and says an invoice attached to a contract with a billing provider will be distributed according to that configuration. Within a supplied JSON object, UUID `id` is required, while the enclosing `requestBody` is not marked required and `additionalProperties` behavior is unspecified. HTTP `200` defines optional top-level `data`; within `data`, UUID `id` is required and described as the new invoice ID. The refreshed request and response examples use different UUIDs, aligning with the prose that the replacement ID is distinct; the schema itself does not encode inequality. The page does not define the new invoice's state or amounts, calculation as-of or atomicity semantics, balance and ledger reconciliation, distribution timing or outcome, downstream voiding, payment or refund effects, errors, or endpoint-specific repeated-call, concurrency, and timeout-recovery behavior. Apply the separate API-wide `Idempotency-Key` contract without assuming a changed key is safe after uncertain regeneration state. Its billing-period wording does not establish that corrected usage bypasses a separately documented historical-submission limit. [[source-metronome-api-reference-invoices-regenerate-an-invoice]]
...
The Product-create schema accepts an optional open string-valued `custom_fields` map without an entity annotation. Dedicated custom-field authority says a configured Product value can be set during object creation and propagates to the associated invoice line, but current Product reads annotate returned maps as `contract_product` and the create-key enum exposes both `product` and `contract_product` without reconciliation. Propagation requires a configured value and does not establish destination precedence, timing, draft-versus-finalized scope, retroactivity, update or deletion behavior, provider acceptance, payment, tax, accounting, export visibility, or reconciliation. [[source-metronome-api-reference-products-create-a-product]] [[source-metronome-api-reference-custom-fields]] [[source-metronome-api-reference-custom-fields-create-a-custom-field-key]] [[source-metronome-api-reference-products-get-a-product]] [[source-metronome-api-reference-products-list-products]]
...
- [[source-metronome-api-reference-invoices-regenerate-an-invoice]] - invoice regeneration contract, recalculation and distribution side effects, refreshed distinct-ID examples, and remaining schema, lineage, and retry boundaries
...
- [[source-metronome-integrations-invoice-integrations-netsuite]] - native NetSuite billing and revenue-system invoice flows, OPEN-to-PAID recovery, sync diagnostics, zero-dollar commit application, and tax ownership boundary

- [[source-metronome-api-reference-sdks]] — introductory contract-to-draft-invoice route whose literal August 2024 usage is stale and whose Go September contract start and missing metric grouping do not establish the claimed prior-usage or user-grouped invoice outcome
