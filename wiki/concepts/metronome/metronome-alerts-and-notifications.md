...
Bearer-authenticated `POST /v1/alerts/archive` permanently disables one threshold-notification configuration selected by required payload UUID `id`, removes it from active monitoring across all customers, and says evaluation stops immediately; the configuration cannot be re-enabled. Optional `release_uniqueness_key: true` resets its resource key for reuse. Current creation authority defines that key as 1-128 characters and organization-unique, says every threshold notification must have one although the payload required array omits it, and says reuse blocks creation with HTTP `409`; the API-wide lifetime table says the key lasts until released. Omitted or false behavior, normalization, release timing and visibility, atomic ordering, failed-attempt consumption, concurrency, and replacement remain undocumented. Data Export separately exposes `alert.archived_at` and `alert.disabled_at` plus alert-linked status-change rows in `customer_alert_history`; its cookbook nevertheless labels a query active while filtering `webhooks_enabled` and `disabled_at` but not `archived_at`, and no authority says archival also sets `disabled_at`. These routes do not prove immediate propagation, history completeness or immutability, retention duration, webhook-delivery history, or a compliance outcome. [[source-metronome-api-reference-alerts-archive-a-threshold-notification]] [[source-metronome-api-reference-alerts-create-a-threshold-notification]] [[source-metronome-guides-reporting-insights-data-export-database-reference]] [[source-metronome-guides-reporting-insights-data-export-cookbook]]
...
### Offset-notification configuration inventory

Bearer-authenticated `POST /v2/notifications/offset/list` inventories user-created offset-notification configurations and separately states a maximum of 400 results per request. It documents optional JSON-body `limit` and `cursor`; `archive_filter` accepts archived, non-archived, or all configurations and defaults to non-archived; HTTP `200` requires `data` and may expose nullable sibling `cursor`. The generic pagination authority instead says all list endpoints expose URL query `limit` and `next_page`, caps `limit` at 100, and repeats until `next_page` is null. The authorities do not establish whether this endpoint accepts, rejects, or ignores those generic query fields, whether the 100 cap governs body `limit`, whether callers may request 400, or whether generic completion semantics apply to response `cursor`; accepted body-limit range and default, cursor terminal semantics, precedence, ordering, totals, lifetime, filter binding, stable snapshot, duplicate-or-skip behavior, as-of time, freshness, and read-after-mutation visibility remain unresolved. Each item requires configuration identity, name, type, environment, creator, creation time, nullable archive time, and a policy with a required lifecycle-event type and signed ISO-8601 offset that distinguishes before-event from after-event schedules. The endpoint defines no event-type catalog, calculated fire time, generation state, delivery state, or history completeness. It lists configurations only and does not establish creation, edit, archival transition, event enablement, generation, webhook delivery, or reconciliation behavior. [[source-metronome-api-reference-notifications-list-offset-lifecycle-event-notification-configurations]]
...
## Sources
- [[source-metronome-api-reference-notifications-archive-an-offset-lifecycle-event-notification-configuration]] - offset-notification configuration archival, the documented nonprocessing effect, already-generated and delivery boundaries, and exact request and response locators
- [[source-metronome-api-reference-notifications-create-an-offset-lifecycle-event-notification-configuration]] - offset lifecycle event notification configuration creation, lifecycle-event type inference from `policy.type`, signed offset navigation, stored-configuration response, and event-emission and delivery boundaries
