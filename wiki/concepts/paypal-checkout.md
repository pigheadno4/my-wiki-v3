...
At `@paypal/checkout-components@5.0.432`, a different accessibility approach adds `aria-label="Payment Button"` to the Buttons iframe, retaining the existing funding-qualified `title` and `role="presentation"`. It does not restore the `5.0.428` title branch or change merchant callbacks. The changelog describes a duplicate-announcement fix; actual assistive-technology behavior has not been tested here. The retained `dist/button.js` lacks this label, so the claim is grounded in authored source, not proof of a deployed bundle. Source: [[source-github-paypal-checkout-components]]. This finding is specific to `5.0.432`; later collected releases require their own ingest.

`@paypal/checkout-components@5.0.433` removes that aria-label and restores the entire Buttons component to its `5.0.431` contents. The funding-qualified title and presentation role remain. The changelog explicitly names the revert but provides no rationale or replacement mitigation. The label is therefore `5.0.432`-only evidence, not current behavior at `5.0.433`. See [[changelog-github-paypal-checkout-components]]. No browser/screen-reader test was performed.

At `@paypal/checkout-components@5.0.434`, the checkout child installs a configurable `window.name` accessor. Zoid-shaped names pass silently; other assignments log `checkout_window_name_overwritten` before updating the stored name. This is monitoring, not an origin/authenticity check or a name-blocking control. Old and new values are truncated to 50 characters, not redacted. Setup errors log `checkout_window_name_spy_error`; the setup catch does not cover subsequent setter calls. No merchant API or dependency-range change is established, and the `5.0.433` Buttons aria-label revert remains. Browser/lifecycle behavior was not tested. See [[source-github-paypal-checkout-components]] and [[changelog-github-paypal-checkout-components]].
