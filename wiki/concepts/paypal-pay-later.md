...
At `1.96.0`, calculator input errors gain explicit invalid/described-by attributes, loading gains a localized status announcement and busy state, decorative shimmers are hidden from assistive technology, and offer-card titles become headings. The legacy message renderer adds a PayPal or PayPal Credit text alternative alongside its logo. These are source-level accessibility changes, not independently tested screen-reader outcomes. New US Apple Wallet-named modal content describes Pay Monthly and Pay in 4 using a single-use virtual card; it does not establish a merchant Apple Pay API or eligibility. The new v2 message fixtures are development content, not production availability evidence. Earlier `1.95.1` findings remain historical evidence. See [[source-github-paypal-messaging-components]] and [[changelog-github-paypal-messaging-components]].

At `1.97.0`, web Messaging Components replaces `pageType: "view-edit-fi"` with `"view-edit-funding-instrument"`; the old value now warns and resolves to undefined rather than being aliased. Apple Wallet-named modal content opts into inline disclosure links, rendered in an iframe with a Back button, and native-webview click callbacks gain the link URL. This is disclosure presentation, not merchant Apple Pay checkout support. Spain and Italy message content adds borrowing-cost warnings with corresponding wrapping and font adjustments; v2 default text size changes from 14px to 12px. These are version-qualified implementation findings, not legal-compliance, eligibility or live-rendering proof. The full additive ingest preserves both earlier releases and includes two approved exact-SHA supplemental files. See [[source-github-paypal-messaging-components]] and [[changelog-github-paypal-messaging-components]].
