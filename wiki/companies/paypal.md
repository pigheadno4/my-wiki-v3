...
The package-qualified v6 surface in `@paypal/paypal-js@9.8.0` supports direct instance creation with client ID or client token, conditional components for PayPal, Venmo, guest payments, messages, subscriptions, Card Fields, Apple Pay, and Google Pay, plus eligibility hydration. `@paypal/react-paypal-js@9.3.0` exposes this integration through the `/sdk-v6` subpath. In the coordinated `10.0.0` releases, both the core v6 loader and React `PayPalProvider` require an explicit `production` or `sandbox` environment; the client ID does not choose the endpoint. Core `10.0.1` adds typed v6 DOM custom elements and legacy Buttons Venmo vault-without-purchase setup-token approval data, while React `10.1.0` requires the same environment choice for server eligibility. Core `10.0.2` corrects `/sdk-v6` resolution for condition-sensitive bundlers; React `10.1.1` only relocates v5 Storybook tooling. Core `10.0.3` adds v6 Venmo save-payment types that conflict with older product availability guidance; React `10.1.2` removes the ineffective Apple Pay disabled prop and types the Messages `TEXT` logo. Core `10.1.0` rejects inherited loader environment values and changes Messages failures to empty content, while React `10.2.0` adds Braintree Messages and corrects server eligibility naming and hydration reuse. React `10.2.1` coordinates server hydration with the client eligibility hook so no-payload SSR consumers do not issue a competing fetch. React `10.3.0` types `merchant_info.merchant_origin` for server eligibility requests. Core `11.0.0` removes PayPal's conflicting `ApplePaySession` global declaration, formalizes the Google Pay 3DS payer-action promise and liability-shift types, and adds component-narrowed v6 TypeScript sessions for 50 local methods. React `10.4.0` adopts core 11, delays Google Pay approval until required 3DS resolves, routes 3DS failure to `onError`, and migrates its Apple Pay hook to the native browser global; these sources still do not establish merchant or regional availability.

The runnable `paypal-examples/v6-web-sdk-sample-integration` history now extends from the full `b5f2df2` baseline to the contained `de90a89` delta. The baseline combines static JavaScript, React 19/TypeScript, and a Node server across standard and advanced PayPal presentation, Venmo, cards, guest checkout, Fastlane, subscriptions, vaulting, Apple Pay, Google Pay with 3DS, and 46 local methods. The delta adds a Basic Apple Pay path where a client-token-initialized PayPal session drives the Apple sheet lifecycle without Apple's separate JavaScript SDK; the recommended merchant-driven flow remains. It also moves the React sample from `@paypal/react-paypal-js ^10.1.0` to `^10.4.0` and native Apple Pay typings. Sample presence proves integration contracts, not merchant or regional eligibility; its local-method README also conflicts with six implementations that explicitly capture after approval.
...
The independent `paypal/paypal-checkout-components` history begins with `@paypal/checkout-components@4.1.47` and includes the following history through `5.0.431` (later releases are summarized above). The v4 runtime implemented Zoid-based Buttons and Checkout with mobile-only secondary Venmo. The accumulated v5 runtime adds separate Card Fields, Payment Fields, Hosted Buttons, Wallet, Saved Payment Methods, Venmo, and QR component boundaries; its Venmo vault-without-purchase path is experiment-gated. The `5.0.426` delta raises the declared development baseline to Node 22/npm 10, upgrades Prettier 3, and moves Zoid to `^10.6.0`; `5.0.427` adds Austria to the eligible Pay Later `Später Bezahlen` label and compact sizing branches and renames rebranded CSS fonts to `PayPalOpen-Regular`; `5.0.428` gives the no-funding-source Buttons iframe the distinct `PayPal Payment Buttons` title, but `5.0.429` reverts that accessibility fix and restores plain `PayPal`; `5.0.430` and `5.0.431` are release-only with no authored source change. None has a detected public export change, and these package-qualified facts do not replace current availability guidance.

The separate `paypal/paypal-sdk-release` baseline records how the combined browser SDK is assembled and promoted. `@paypal/sdk-release@5.0.569` pins twelve direct components, including Checkout Components `5.0.428`, Messaging Components `1.94.0`, Apple Pay Components `1.8.2`, Google Pay Components `1.3.5`, and SDK Client `4.0.204`. This is component bill-of-materials evidence, not merchant eligibility or component behavior; its checkout version contains the short-lived title fix whereas the separately ingested `5.0.429` reverts that title change.
...
The `1.96.0` delta adds calculator error/loading accessibility semantics, financing-plan headings and a logo text alternative. Its US Apple Wallet-named modal content describes single-use virtual-card financing, not a new merchant Apple Pay API. V2 message fixtures and demo guidance remain development evidence; no live eligibility or runtime accessibility proof is inferred. See [[source-github-paypal-messaging-components]] and [[changelog-github-paypal-messaging-components]].
