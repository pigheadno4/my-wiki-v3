# Historical Metronome provider rules

Archived on 2026-09-08 during an approved instruction-entry cleanup. The original
provider file is preserved verbatim below for historical campaign interpretation;
its dated permissions are not new execution authority. Do not load this file for
routine new campaigns. Use [metronome.md](metronome.md) and its selected workflow.
Keeping this archive in the same directory preserves original relative links.
...
- For concept suggestions, set `update_kind` to `durable_fact` or `reciprocal_source_link` only. Preserve a contradiction in `proposed_markdown` or `warnings`; do not invent a new update kind for it.
