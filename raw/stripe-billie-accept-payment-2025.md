<!-- Source URL: https://docs.stripe.com/payments/billie/accept-a-payment -->
<!-- Fetched: 2026-05-06 -->
...
> This is a Checkout for when payment-ui is checkout. View the full page at https://docs.stripe.com/payments/billie/accept-a-payment?payment-ui=checkout.
...
- The total amount must be at least 0,01 EUR.
...
const session = await stripe.checkout.sessions.create({
  ...
  line_items: [
    {
      price_data: {
        // To accept `billie`, all line items must have currency:eur, sek, nok, dkk, gbp, chfcurrency: 'eur',
        product_data: {
          name: "T-shirt",
        },
        unit_amount: 1000,
...
![Billie payment method visible in Stripe Checkout](assets/stripe-billie-checkout-visible.png)

## See also

- [More about Billie](https://docs.stripe.com/payments/billie.md)
...
> This is a Checkout Sessions API for when payment-ui is elements and api-integration is checkout. View the full page at https://docs.stripe.com/payments/billie/accept-a-payment?payment-ui=elements&api-integration=checkout.
...
- Directly before [submitting the payment](https://docs.stripe.com/payments/billie/accept-a-payment.md#submit-payment). You can also call `updateEmail` to validate earlier, such as on input blur.
...
> This is a Payment Intents API for when payment-ui is elements and api-integration is paymentintents. View the full page at https://docs.stripe.com/payments/billie/accept-a-payment?payment-ui=elements&api-integration=paymentintents.
...
To indicate that you want separate authorization and capture, set [capture_method](https://docs.stripe.com/api/payment_intents/create.md#create_payment_intent-capture_method) to `manual` when creating the PaymentIntent. This parameter instructs Stripe to only authorize the amount on the customer’s Billie account.
...
> This is a Direct API for when payment-ui is direct-api. View the full page at https://docs.stripe.com/payments/billie/accept-a-payment?payment-ui=direct-api.
...
Use the [client secret](https://docs.stripe.com/api/payment_intents/object.md#payment_intent_object-client_secret) of the `PaymentIntent` and call `stripe.confirmPayment` to handle the Billie redirect. Add a `return_url` to determine where Stripe redirects the customer after they complete the payment.
...
> This is a iOS for when payment-ui is mobile and platform is ios. View the full page at https://docs.stripe.com/payments/billie/accept-a-payment?payment-ui=mobile&platform=ios.

Use the [Mobile Payment Element](https://docs.stripe.com/payments/accept-a-payment.md?=ios), an embeddable payment form, to add Billie and other payment methods to your integration with the least amount of effort.
...
When a customer taps to pay with Billie, confirm the `PaymentIntent` to complete the payment. Configure an `STPPaymentIntentParams` object with the `PaymentIntent` [client secret](https://docs.stripe.com/api/payment_intents/object.md#payment_intent_object-client_secret).
...
The iOS SDK presents a webview in your app to complete the Billie payment. When authentication is finished, the webview can automatically dismiss itself instead of having your customer close it. To enable this behavior, configure a custom URL scheme or universal link and set up your app delegate to forward the URL to the SDK.
...
### Confirm the Billie payment

Complete the payment by calling `STPPaymentHandler.confirmPayment`. This presents a webview where the customer can complete the payment with Billie. After completion, Stripe calls the completion block with the result of the payment.
...
// Billie does not require additional parameters so we only need to pass the initialized
// STPPaymentMethodBillieParams instance to STPPaymentMethodParams
let billie = STPPaymentMethodBillieParams()
let paymentMethodParams = STPPaymentMethodParams(billie: billie, billingDetails: nil, metadata: nil)
...
> This is a Android for when payment-ui is mobile and platform is android. View the full page at https://docs.stripe.com/payments/billie/accept-a-payment?payment-ui=mobile&platform=android.

Use the [Mobile Payment Element](https://docs.stripe.com/payments/accept-a-payment.md?=android), an embeddable payment form, to add Billie and other payment methods to your integration with the least amount of effort.
...
Complete the payment by calling `PaymentHandler.confirmPayment`. This presents a webview where the customer can complete the payment with Billie. After completion, Stripe calls the completion block with the result of the payment.
...
class CheckoutActivity : AppCompatActivity() {
    ...
    private fun startCheckout() {
        ...
        val billieParams = PaymentMethodCreateParams.createBillie()
