---
title: Integrate EPS using the JavaScript SDK
slug: /docs/checkout/apm/eps/js-sdk/
createTime: "2024-12-09T09:36:34.341Z"
updateTime: "2025-03-28T03:25:35.181Z"
---

# Integrate EPS using the JavaScript SDK
...
```javascript
<script src="https://www.paypal.com/sdk/js?client-id=YOUR_CLIENT_ID&components=buttons,payment-fields,marks,funding-eligibility&enable-funding=eps&currency=EUR"></script>
```
...
![image](assets/paypal-apm-eps-mark.png)
...
For EPS, payment fields collect first name and last name.

If there are validation errors in the input fields, they'll show on the click of the button.

#### **`eps payments Single page checkout flow`**
...
![image](assets/paypal-apm-multipage-1.png)
  ...
  .PaymentFields({
    ...
    onInit: (data, actions) => {
      ...
      form.addEventListener("submit", (e) => {
        ...
        if (paymentSource === paypal.FUNDING.EPS) {
          e.preventDefault();

          actions.validate().then((valid) => {
            if (valid) {
              window.location.href = `/second-page.html?payment-option=${paypal.FUNDING.EPS}`;
            }
          });
        }
      });
...
![image](assets/paypal-apm-multipage-2.png)
...
- createOrder Implement the createOrder function to allow the JavaScript SDK to submit buyer information and set up the transaction on the click of the button.
  ...
  **Note:** Create EPS orders in EUR currency.
...
The functioncaptures the funds from the transaction and shows a message to the buyer to let them know the transaction is successful. The method is called after the buyer approves the transaction on paypal.com.
...
- Webhook events - [Checkout webhook events](https://developer.paypal.com/api/rest/webhooks/event-names/) - Checkout buyer approval-related webhooks.
...
See a sample EPS integration in the [PayPal GitHub repository](https://github.com/paypal-examples/eps-paypal-payment-js-sdk) .
