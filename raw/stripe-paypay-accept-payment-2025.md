<!-- Source URL: https://docs.stripe.com/payments/paypay/accept-a-payment -->
<!-- Fetched: 2026-05-07 -->

# Accept a payment with PayPay

Learn how to set up your integration with PayPay.

# Checkout

> This is a Checkout for when payment-ui is checkout. View the full page at https://docs.stripe.com/payments/paypay/accept-a-payment?payment-ui=checkout.
...
PayPay is a single-use payment method that require customers to authenticate their payment. Customers pay by being redirected from your website or app, authorizing the payment with PayPay, then returning to your website or app. You get immediate notification of whether the payment succeeded or failed.
...
To support PayPay payments, a Checkout Session must satisfy all of the following conditions:
...
This guide describes how to enable PayPay and shows the differences between accepting a card payment and using PayPay.

### Enable PayPay as a payment method

When creating a new [Checkout Session](https://docs.stripe.com/api/checkout/sessions.md), you need to:

1. Add `paypay` to the list of `payment_method_types`.
...
const session = await stripe.checkout.sessions.create({
  ...
  line_items: [
    {
      price_data: {
        // To accept `paypay`, all line items must have currency:jpycurrency: 'jpy',
        product_data: {
          name: "T-shirt",
        },
        unit_amount: 1000,
...
When testing your Checkout integration, select PayPay as the payment method and click the **Pay** button.

## See also

- [More about PayPay](https://docs.stripe.com/payments/paypay.md)
...
> This is a Checkout Sessions API for when payment-ui is elements and api-integration is checkout. View the full page at https://docs.stripe.com/payments/paypay/accept-a-payment?payment-ui=elements&api-integration=checkout.
...
- Directly before [submitting the payment](https://docs.stripe.com/payments/paypay/accept-a-payment.md#submit-payment). You can also call `updateEmail` to validate earlier, such as on input blur.
...
In live mode, tapping **Pay** redirects you to the PayPay website—you don’t have the option to approve or decline the payment with PayPay.

# Payment Intents API

> This is a Payment Intents API for when payment-ui is elements and api-integration is paymentintents. View the full page at https://docs.stripe.com/payments/paypay/accept-a-payment?payment-ui=elements&api-integration=paymentintents.
...
> This is a Direct API for when payment-ui is direct-api. View the full page at https://docs.stripe.com/payments/paypay/accept-a-payment?payment-ui=direct-api.
...
Use the [client secret](https://docs.stripe.com/api/payment_intents/object.md#payment_intent_object-client_secret) of the `PaymentIntent` and call `stripe.confirmPayment` to handle the PayPay redirect. Add a `return_url` to determine where Stripe redirects the customer after they complete the payment.
...
> This is a iOS for when payment-ui is mobile and platform is ios. View the full page at https://docs.stripe.com/payments/paypay/accept-a-payment?payment-ui=mobile&platform=ios.

Use the [Mobile Payment Element](https://docs.stripe.com/payments/accept-a-payment.md?=ios), an embeddable payment form, to add PayPay and other payment methods to your integration with the least amount of effort.
...
When a customer taps to pay with PayPay, confirm the `PaymentIntent` to complete the payment. Configure an `STPPaymentIntentParams` object with the `PaymentIntent` [client secret](https://docs.stripe.com/api/payment_intents/object.md#payment_intent_object-client_secret).
...
The iOS SDK presents a webview in your app to complete the PayPay payment. When authentication is finished, the webview can automatically dismiss itself instead of having your customer close it. To enable this behavior, configure a custom URL scheme or universal link and set up your app delegate to forward the URL to the SDK.
...
### Confirm the PayPay payment

Complete the payment by calling `STPPaymentHandler.confirmPayment`. This presents a webview where the customer can complete the payment with PayPay. After completion, Stripe calls the completion block with the result of the payment.
...
// Paypay does not require additional parameters so we only need to pass the initialized
// STPPaymentMethodPaypayParams instance to STPPaymentMethodParams
let paypay = STPPaymentMethodPaypayParams()
let paymentMethodParams = STPPaymentMethodParams(paypay: paypay, billingDetails: nil, metadata: nil)
...
> This is a Android for when payment-ui is mobile and platform is android. View the full page at https://docs.stripe.com/payments/paypay/accept-a-payment?payment-ui=mobile&platform=android.

Use the [Mobile Payment Element](https://docs.stripe.com/payments/accept-a-payment.md?=android), an embeddable payment form, to add PayPay and other payment methods to your integration with the least amount of effort.
...
Complete the payment by calling `PaymentHandler.confirmPayment`. This presents a webview where the customer can complete the payment with PayPay. After completion, Stripe calls the completion block with the result of the payment.
