...
<!-- Fetched: 2026-04-13 -->
<!-- Tab: Server Side Shipping Callback (variant of paypal-checkout-integrate-one-time-payment.md) -->
...
### Step 5. Support multiple shipping options (OPTIONAL) — Server Side Shipping Callback

The server-side shipping callbacks allow you to update the shipping and order amount information as buyers make changes on the PayPal review page.
...
Visit the Shipping module page and configure shipping preferences for more details about server-side shipping callbacks.

#### Merchant Decline Response

A merchant can decline a callback event with a 422 error due to rejected shipping address, server-side request failure or timeout or poorly formatted response.

To simulate error scenario, set the City at checkout to one of the following values:
...
1. Log in to the PayPal sandbox using the sandbox business account that received the payment.
...
### server.js (Node.js backend) — with server-side shipping callbacks
