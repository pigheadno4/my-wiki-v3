...
![Four smartphone screenshots demonstrating a one-click checkout integration with one-time payments for retail purchases.](assets/paypal-best-practices-one-time-overview.png)
...
![Screenshot showing example PayPal button placement on cart page.](assets/paypal-best-practices-one-time-placement-cart.png)
...
![Screenshot showing example PayPal button placement on product detail page.](assets/paypal-best-practices-one-time-placement-pdp.png)
...
> **Tip:** Use the shipping options callback for physical goods even when only one delivery method is available.
...
![Screenshot showing example PayPal button on merchant checkout page.](assets/paypal-best-practices-one-time-checkout-page.png)
...
![Screenshot showing example PayPal login/loading page.](assets/paypal-best-practices-one-time-loading-page.png)
...
- Implement App Switch to redirect buyers to the PayPal app for streamlined authentication and checkout completion.

![Screenshot showing example PayPal Checkout paysheet/review page.](assets/paypal-best-practices-one-time-paysheet.png)

#### Pay Now flow

![A diagram showing the Pay Now payment flow.](assets/paypal-best-practices-one-time-pay-now-flow.png)

## Next steps

- **Message placement** — Render effective messaging with message placement and product amount.
- **Shipping Module** — Present shipping details to a buyer during the PayPal flow.
- **Contact Module** — Help buyers add or modify contact information during checkout.
- **Pass buyer identifier** — Prefill a buyer's PayPal login page by passing their email address.
- **Configure the data-page-type** — Use the JavaScript SDK to configure the data-page-type.
- **Create order** — Create an order using the Orders v2 API.
- **Pass line-item details** — Manage line-item details using the Orders v2 API.
