<!-- Source: Stripe Checkout — Fulfill orders -->
<!-- Fetched: 2026-04-20 -->
...
The automatic fulfillment system outlined below uses a combination of _webhooks_ (A webhook is a real-time push notification sent to your application as a JSON payload through HTTPS requests) and a redirect to your website to trigger fulfillment. You must use webhooks to make sure fulfillment happens for every payment, and redirects let your customers access services or fulfillment details immediately after paying.
...
For Checkout Sessions with the default [ui_mode](https://docs.stripe.com/api/checkout/sessions/create.md#create_checkout_session-ui_mode) of `hosted_page`, set the `success_url`.
