...
1. In the Metronome UI, navigate to **Connections > API tokens & webhooks**.
2. Click **+ Add**.
3. Select a role to assign to the token (**Administrator**, **Writer**, or **Reader**).
4. Complete token creation.

<Note>
  To assign a custom role to an API token, contact your Metronome representative to have the role created first.
</Note>
