---
title: Integrate Przelewy24 using the JavaScript SDK
slug: /docs/checkout/apm/przelewy24/js-sdk/
createTime: "2024-12-06T09:29:19.536Z"
updateTime: "2025-03-18T19:59:01.063Z"
---

# Integrate Przelewy24 using the JavaScript SDK
...
```javascript
<script src="https://www.paypal.com/sdk/js?client-id=YOUR_CLIENT_ID&components=buttons,payment-fields,marks,funding-eligibility&enable-funding=p24&currency=EUR"></script>
```
...
You can choose from the followingcheckout flows:
...
For Przelewy24, payment fields collect first name, last name, and email.
  ...
  .PaymentFields({
    ...
    onInit: (data, actions) => {
      ...
      form.addEventListener("submit", (e) => {
        ...
        if (paymentSource === paypal.FUNDING.P24) {
          e.preventDefault();

          actions.validate().then((valid) => {
            if (valid) {
              window.location.href = `/second-page.html?payment-option=${paypal.FUNDING.P24}`;
            }
          });
        }
      });
...
#### **`paypal.Buttons /docs/checkout/apm/przelewy24/js-sdk`**
...
- createOrder Implement the createOrder function to allow the JavaScript SDK to submit buyer information and set up the transaction on the click of the button.
  ...
  **Note:**Create Przelewy24 orders in PLN/EUR currency. Use your server-side [Create order](/docs/api/orders/v2/#orders_create) call to set up the details of a one-time transaction including the amount, line item detail, and more. If order creation fails, the Orders API can [return an error](https://developer.paypal.com/api/rest/reference/orders/v2/errors/) in the console. After order creation, orders are confirmed with buyer-selected payment source. If the order cannot be processed with the selected payment source, the relevant errors are returned in the console.
...
See a sample Przelewy24 integration in the [PayPal GitHub repository](https://github.com/paypal-examples/p24-paypal-payment-js-sdk) .
