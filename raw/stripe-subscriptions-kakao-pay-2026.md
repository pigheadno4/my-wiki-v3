<!-- Source URL: https://docs.stripe.com/billing/subscriptions/kakao-pay -->
<!-- Fetched: 2026-05-13 -->

# Set up a subscription with Kakao Pay

Set up a _subscription_ (A Subscription represents the product details associated with the plan that your customer subscribes to. Allows you to charge the customer on a recurring basis) using [Kakao Pay](https://docs.stripe.com/payments/kakao-pay/accept-a-payment.md) as a payment method.

# SetupIntents API

> This is a SetupIntents API for when api-integration is setupintents. View the full page at https://docs.stripe.com/billing/subscriptions/kakao-pay?api-integration=setupintents.
...
curl https://api.stripe.com/v1/setup_intents \
  ...
  -d "payment_method_data[type]=kakao_pay" \
  -d "payment_method_types[]=kakao_pay" \
...
Confirm a payment with [PaymentIntents](https://docs.stripe.com/api/payment_intents.md) using the PaymentIntent ID from the Subscriptions response. Add the PaymentIntent ID to the URL path and set the value of the `payment_method_types` parameter to `kakao_pay`:
...
curl https://api.stripe.com/v1/payment_intents/:id/confirm \
  ...
  -d "payment_method_data[]=kakao_pay"
```
...
> This is a Subscriptions API for when api-integration is subscription. View the full page at https://docs.stripe.com/billing/subscriptions/kakao-pay?api-integration=subscription.

Create and confirm a Subscription using two API calls.

- First, [create a subscription](https://docs.stripe.com/billing/subscriptions/kakao-pay.md#pi-create-subscription) to sends customer and product information to the [Subscriptions API](https://docs.stripe.com/api/subscriptions.md) to create a Subscription and PaymentIntent in one call.
- Second, use the PaymentIntent ID to [confirm a payment](https://docs.stripe.com/billing/subscriptions/kakao-pay.md#pi-confirm-payment).
...
The PaymentIntent response includes the status `requires_action`, which indicates that your customers must authenticate with Kakao Pay to complete the PaymentIntent. After a successful payment, the subscription becomes active and saves the payment method as the default payment method.

# Stripe-hosted page

> This is a Stripe-hosted page for when api-integration is checkout. View the full page at https://docs.stripe.com/billing/subscriptions/kakao-pay?api-integration=checkout.

Use the [Checkout Sessions API](https://docs.stripe.com/api/checkout/sessions.md) to create and confirm a subscription with a prebuilt checkout page.
...
Your customer must authorize you to use their Kakao Pay account for future payments through Stripe Checkout. This allows you to accept Kakao Pay payments. Add a checkout button to your website that calls a server-side endpoint to create a [Checkout Session](https://docs.stripe.com/api/checkout/sessions.md).
...
Select Kakao Pay as the payment method and tap Subscribe. You can test the successful payment case by authenticating the payment on the redirect page. The PaymentIntent transitions from `requires_action` to `succeeded`.
