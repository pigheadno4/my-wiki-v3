<!-- Source URL: https://docs.stripe.com/radar/rules/reference -->
<!-- Fetched: 2026-05-10 -->
...
To understand how Radar transaction rules work with Stripe _Connect_ (Connect is Stripe's solution for multi-party businesses, such as marketplace or software platforms, to route payments between sellers, customers, and other recipients), see [Using Radar with Connect](https://docs.stripe.com/connect/radar.md).
...
_Radar_ (Stripe Radar helps detect and block fraud for any type of business using machine learning that trains on data across millions of global companies. It’s built into Stripe and requires no additional setup to get started) evaluates each payment against the rules you create according to their action type, using the following prioritization:

1. **Request 3DS**: Rules that when used with the [Payment Intents API](https://docs.stripe.com/payments/payment-intents.md) or [Checkout](https://docs.stripe.com/payments/checkout.md), _request_ the issuer to do _3D Secure authentication_ (3D Secure (3DS) provides an additional layer of authentication for credit card transactions that protects businesses from liability for fraudulent card payments) if supported. Radar evaluates these before any block, review, or allow rules.
1. **Allow**: Rules that _allow_ a payment to be processed. Payments that fall under allow rules aren’t evaluated against any block or review rules.
1. **Block**: Rules that _block_ a payment and reject it. Blocked payments aren’t evaluated against any review rules.
1. **Review**: Rules that allow payments to be processed but then place them in _review_.
...
The rule structure has two components—the _action_ to take and the _condition_ to evaluate:

`{action} if {condition}`

Together they represent the _predicate_. In practice, a rule to block all payments over 1,000 USD appears as:
...
When used with the Payment Intents API, this rule determines if Stripe requests the issuer to attempt _3D Secure authentication_ (3D Secure (3DS) provides an additional layer of authentication for credit card transactions that protects businesses from liability for fraudulent card payments). Requesting 3DS alone doesn’t block all possible [3D Secure outcomes](https://docs.stripe.com/radar/rules.md#request-3d-secure). Whether or not there are matches on this rule, we evaluate rules for allow, block, and review afterward.
...
- **Attribute**: The attribute of a payment (for example, the _amount_ or _payment method_)
...
Address (_AVS_ (The address verification system (AVS) is used to pass billing address information to issuers to verify the data if available)) and _CVC_ (The card verification code (CVC) or card verification value (CVV) is a three- or four-digit number printed directly on a card used to verify the entered card number) attributes have five possible values:
...
Requiring a strict `pass` on rules can be overly restrictive. For example, _wallets_ (A digital wallet is a contactless payment method that stores payment options, such as credit and debit cards, allowing customers to use a smart device to make a purchase) usually don’t provide a CVC because they store tokenized card information. Therefore _CVC_ (The card verification code (CVC) or card verification value (CVV) is a three- or four-digit number printed directly on a card used to verify the entered card number) checks, like _3D-Secure_ (3D Secure (3DS) provides an additional layer of authentication for credit card transactions that protects businesses from liability for fraudulent card payments) checks, aren’t available for payment methods such as Apple Pay. Stripe recommends using Radar’s [Built-in rules](https://docs.stripe.com/radar/rules.md#built-in-rules), which consider these edge cases.
