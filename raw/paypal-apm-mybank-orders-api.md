---
title: Integrate MyBank using the Orders API
slug: /docs/checkout/apm/mybank/orders-api/
createTime: "2024-12-05T11:23:11.340Z"
updateTime: "2025-05-06T01:47:47.386Z"
---

# Integrate MyBank using the Orders API
...
- When processing MyBank payments, you don't need to capture payment for the order.

## Offer MyBank on the checkout page

You'll need to create the user interface to offer MyBank and collect the buyer's information, then you'll use the API calls described in the remainder of this topic to:

- Create the order with MyBank as the payment source and buyer's full_name and country_code
- Redirect the buyer to MyBank

Refer to [Payment method icons](/docs/checkout/apm/reference/method-icons/) for icons you can use and download locations.

Use the buyer information you captured from your user interface to create an order with MyBank as the payment source.
...
- payment_source - Specify mybank as the payment method and include the country code and account holder's full name.
...
**Note:**Change or add other parameters in the[Create order](/docs/api/orders/v2/#orders_create)request body to create an order that reflects the actual order details.
...
```javascript
{
    ...
    "links": [
        ...
        {
            "href": "https://www.sandbox.paypal.com/payment/mybank?token=1BV95196WW992540E",
            "rel": "payer-action",
            "method": "GET"
        }
    ]
...
In your user interface, attach the payer-action redirect URL returned in theCreate Orderresponse to the MyBank payment button. This sends the buyer to their bank to approve the purchase. Once the buyer approves the purchase, the payment is automatically captured.

In the sample, the redirect URL is: "https://www.sandbox.paypal.com/payment/mybank?token=1BV95196WW992540E"
...
#### **`Sample CHECKOUT.ORDER.DENIED webhook `**
...
**Notes:**- order ID from [Step 2](#create-an-order-with-mybank-as-the-payment-source) should match resource.supplementary_data.related_ids.order_id parameter in the webhook payload.

- When an order is declined, CHECKOUT.ORDER.DECLINED webhook passes the error code and message in themost_recent_errorparameter of thepurchase_unitobject.
