<!-- Source URL: https://docs.stripe.com/payments/mondu/accept-a-payment -->
<!-- Fetched: 2026-05-06 -->

# Accept a payment with Mondu

Learn how to set up your integration with Mondu.

# Checkout

> This is a Checkout for when payment-ui is checkout. View the full page at https://docs.stripe.com/payments/mondu/accept-a-payment?payment-ui=checkout.
...
Mondu is a [single-use](https://docs.stripe.com/payments/payment-methods.md#usage) payment method that offers customers payment 30 day net terms. Customers are redirected from your website or app, authorize the payment with Mondu, then return to your website or app. You get [immediate notification](https://docs.stripe.com/payments/payment-methods.md#payment-notification) of whether the payment succeeded or failed.
...
1. Add `mondu` to the list of `payment_method_types`.
...
const session = await stripe.checkout.sessions.create({
  ...
  line_items: [
    {
      price_data: {
        // To accept `mondu`, all line items must have currency:eur, chf, gbpcurrency: 'eur',
        product_data: {
          name: "T-shirt",
        },
        unit_amount: 1000,
...
![Mondu payment method visible in Stripe Checkout](assets/stripe-mondu-checkout-visible.png)

## See also

- [More about Mondu](https://docs.stripe.com/payments/mondu.md)
...
> This is a Checkout Sessions API for when payment-ui is elements and api-integration is checkout. View the full page at https://docs.stripe.com/payments/mondu/accept-a-payment?payment-ui=elements&api-integration=checkout.
...
- Directly before [submitting the payment](https://docs.stripe.com/payments/mondu/accept-a-payment.md#submit-payment). You can also call `updateEmail` to validate earlier, such as on input blur.
...
In live mode, tapping **Pay** redirects you to the Mondu website—you don’t have the option to approve or decline the payment with Mondu.

# Payment Intents API

> This is a Payment Intents API for when payment-ui is elements and api-integration is paymentintents. View the full page at https://docs.stripe.com/payments/mondu/accept-a-payment?payment-ui=elements&api-integration=paymentintents.
...
To indicate that you want separate authorization and capture, set [capture_method](https://docs.stripe.com/api/payment_intents/create.md#create_payment_intent-capture_method) to `manual` when creating the PaymentIntent. This parameter instructs Stripe to only authorize the amount on the customer’s Mondu account.
...
> This is a Direct API for when payment-ui is direct-api. View the full page at https://docs.stripe.com/payments/mondu/accept-a-payment?payment-ui=direct-api.
...
Use the [client secret](https://docs.stripe.com/api/payment_intents/object.md#payment_intent_object-client_secret) of the `PaymentIntent` and call `stripe.confirmPayment` to handle the Mondu redirect. Add a `return_url` to determine where Stripe redirects the customer after they complete the payment.
