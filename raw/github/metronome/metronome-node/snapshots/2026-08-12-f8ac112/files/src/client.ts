...
export class Metronome {
   ...
   */
  constructor({
    baseURL = readEnv('METRONOME_BASE_URL'),
    bearerToken = readEnv('METRONOME_BEARER_TOKEN'),
    webhookSecret = readEnv('METRONOME_WEBHOOK_SECRET') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? Metronome.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    const customHeadersEnv = readEnv('METRONOME_CUSTOM_HEADERS');
    if (customHeadersEnv) {
