...
This guide outlines the changes and steps needed to migrate your codebase to the latest version of the Metronome TypeScript SDK.
...
To use it, upgrade the `@metronome/sdk` package, then run `./node_modules/.bin/metronome-sdk migrate ./your/src/folders` to update your code.
...
The `httpAgent` client option has been removed in favor of a [platform-specific `fetchOptions` property](https://github.com/Metronome-Industries/metronome-node#fetch-options).
This change was made as `httpAgent` relied on `node:http` agents which are not supported by any runtime's builtin fetch implementation.

If you were using `httpAgent` for proxy support, check out the [new proxy documentation](https://github.com/Metronome-Industries/metronome-node#configuring-proxies).
...
#### Refactor of `@metronome/sdk/core`, `error`, `pagination`, `resource` and `uploads`

Much of the `@metronome/sdk/core` file was intended to be internal-only but it was publicly accessible, as such it has been refactored and split up into internal and public files, with public-facing code moved to a new `core` folder and internal code moving to the private `internal` folder.
...
```typescript
// Before
import '@metronome/sdk/error';
import '@metronome/sdk/pagination';
import '@metronome/sdk/resource';
import '@metronome/sdk/uploads';

// After
import '@metronome/sdk/core/error';
import '@metronome/sdk/core/pagination';
import '@metronome/sdk/core/resource';
import '@metronome/sdk/core/uploads';
```

If you were relying on anything that was only exported from `@metronome/sdk/core` and is also not accessible anywhere else, please open an issue and we'll consider adding it to the public API.
...
As part of the `core` refactor, `@metronome/sdk/uploads` was moved to `@metronome/sdk/core/uploads`
...
```typescript
import { type Uploadable, toFile } from '@metronome/sdk/core/uploads';
```
...
```ts
// Before
Metronome.fileFromPath('path/to/file');
...
The `@metronome/sdk/shims` imports have been removed. Your global types must now be [correctly configured](#minimum-types-requirements).
...
### `@metronome/sdk/src` directory removed

Previously IDEs may have auto-completed imports from the `@metronome/sdk/src` directory, however this
directory was only included for an improved go-to-definition experience and should not have been used at runtime.

If you have any `@metronome/sdk/src/*` imports, you will need to replace them with `@metronome/sdk/*`.

```ts
// Before
import Metronome from '@metronome/sdk/src';
