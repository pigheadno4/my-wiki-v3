<!-- Source URL: https://docs.stripe.com/payments/sunbit/accept-a-payment -->
<!-- Fetched: 2026-05-06 -->

# Accept a payment with Sunbit

Learn how to set up your integration with Sunbit.

# Checkout

> This is a Checkout for when payment-ui is checkout. View the full page at https://docs.stripe.com/payments/sunbit/accept-a-payment?payment-ui=checkout.
...
To support Sunbit payments, a Checkout Session must satisfy all of the following conditions:
...
- The total amount must be between 60.00 USD and 19,999.99 USD.
...
This guide describes how to enable Sunbit and shows the differences between accepting a card payment and using Sunbit.
...
1. Add `sunbit` to the list of `payment_method_types`.
1. Make sure all `line_items` use the same currency.

#### Node.js

```javascript
const session = await stripe.checkout.sessions.create({payment_method_types: ['card', 'sunbit'],
  line_items: [{
    price_data: {
      currency: 'usd
      ',
...
When testing your Checkout integration, select Sunbit as the payment method and click the **Pay** button.
![Sunbit payment method visible in Stripe Checkout](assets/stripe-sunbit-checkout-visible.png)

## See also

- [More about Sunbit](https://docs.stripe.com/payments/sunbit.md)
...
> This is a Checkout Sessions API for when payment-ui is elements and api-integration is checkout. View the full page at https://docs.stripe.com/payments/sunbit/accept-a-payment?payment-ui=elements&api-integration=checkout.
...
- Directly before [submitting the payment](https://docs.stripe.com/payments/sunbit/accept-a-payment.md#submit-payment). You can also call `updateEmail` to validate earlier, such as on input blur.
...
In live mode, tapping **Pay** redirects you to the Sunbit website—you don’t have the option to approve or decline the payment with Sunbit.

# Payment Intents API

> This is a Payment Intents API for when payment-ui is elements and api-integration is paymentintents. View the full page at https://docs.stripe.com/payments/sunbit/accept-a-payment?payment-ui=elements&api-integration=paymentintents.
...
> This is a Direct API for when payment-ui is direct-api. View the full page at https://docs.stripe.com/payments/sunbit/accept-a-payment?payment-ui=direct-api.
...
Use the [client secret](https://docs.stripe.com/api/payment_intents/object.md#payment_intent_object-client_secret) of the `PaymentIntent` and call `stripe.confirmPayment` to handle the Sunbit redirect. Add a `return_url` to determine where Stripe redirects the customer after they complete the payment.
...
> This is a iOS for when payment-ui is mobile and platform is ios. View the full page at https://docs.stripe.com/payments/sunbit/accept-a-payment?payment-ui=mobile&platform=ios.

Use the [Mobile Payment Element](https://docs.stripe.com/payments/accept-a-payment.md?=ios), an embeddable payment form, to add Sunbit and other payment methods to your integration with the least amount of effort.
...
When a customer taps to pay with Sunbit, confirm the `PaymentIntent` to complete the payment. Configure an `STPPaymentIntentParams` object with the `PaymentIntent` [client secret](https://docs.stripe.com/api/payment_intents/object.md#payment_intent_object-client_secret).
...
The iOS SDK presents a webview in your app to complete the Sunbit payment. When authentication is finished, the webview can automatically dismiss itself instead of having your customer close it. To enable this behavior, configure a custom URL scheme or universal link and set up your app delegate to forward the URL to the SDK.
...
### Confirm the Sunbit payment

Complete the payment by calling `STPPaymentHandler.confirmPayment`. This presents a webview where the customer can complete the payment with Sunbit. After completion, Stripe calls the completion block with the result of the payment.
...
// Sunbit does not require additional parameters so we only need to pass the initialized
// STPPaymentMethodSunbitParams instance to STPPaymentMethodParams
let sunbit = STPPaymentMethodSunbitParams()
let paymentMethodParams = STPPaymentMethodParams(sunbit: sunbit, billingDetails: nil, metadata: nil)
...
> This is a Android for when payment-ui is mobile and platform is android. View the full page at https://docs.stripe.com/payments/sunbit/accept-a-payment?payment-ui=mobile&platform=android.

Use the [Mobile Payment Element](https://docs.stripe.com/payments/accept-a-payment.md?=android), an embeddable payment form, to add Sunbit and other payment methods to your integration with the least amount of effort.
...
Complete the payment by calling `PaymentHandler.confirmPayment`. This presents a webview where the customer can complete the payment with Sunbit. After completion, Stripe calls the completion block with the result of the payment.
...
class CheckoutActivity : AppCompatActivity() {
    ...
    private fun startCheckout() {
        ...
        val sunbitParams = PaymentMethodCreateParams.createSunbit()
