---
title: Integrate BLIK using the JavaScript SDK
slug: /docs/checkout/apm/blik/js-sdk/
createTime: "2024-12-10T09:59:32.176Z"
updateTime: "2025-04-25T07:15:23.611Z"
---

# Integrate BLIK using the JavaScript SDK

Merchants can use PayPal-hosted UI components, called payment fields, to collect payment information for alternative payment methods. Use the JavaScript SDK to render payment fields and buttons and process payments with the Orders API.
...
- Request approval to enable BLIK by visiting the following sandbox and live links: - Sandbox: [https://www.sandbox.paypal.com/bizsignup/entry?product=blik&capabilities=BLIK&country.x=&lt;merchant's country&gt;](https://www.sandbox.paypal.com/bizsignup/entry?product=blik&capabilities=BLIK&country.x=&_ga=1.66613998.1793812298.1735285121)
- Live: [https://www.paypal.com/bizsignup/add-product?product=blik&capabilities=BLIK&country.x=&lt;merchant's country&gt;](https://www.paypal.com/bizsignup/add-product?product=blik&capabilities=BLIK&country.x=&_ga=1.66613998.1793812298.1735285121)
...
- Complete the steps in [Get started](https://developer.paypal.com/api/rest/) to get your sandbox account information from the Developer Dashboard: - **Client ID** : Authenticates your account with PayPal and identifies an app in your sandbox.
...
#### **`Add PayPal JavaScript SDK code `**

```javascript
<script src="https://www.paypal.com/sdk/js?client-id=YOUR_CLIENT_ID&components=buttons,payment-fields,marks,funding-eligibility&enable-funding=blik&currency=PLN"></script>
```
...
![image](assets/paypal-apm-blik-mark.png)
  ...
  .PaymentFields({
    ...
    onInit: (data, actions) => {
      ...
      form.addEventListener("submit", (e) => {
        ...
        if (paymentSource === paypal.FUNDING.BLIK) {
          e.preventDefault();

          actions.validate().then((valid) => {
            if (valid) {
              window.location.href = `/second-page.html?payment-option=${paypal.FUNDING.BLIK}`;
            }
          });
        }
      });
...
![Multipage,Flow,Page,2](assets/paypal-apm-multipage-2.png)

For style parameters, please reference this style page: [Custom style for payment fields](/docs/checkout/apm/reference/style/)

#### **`Renderpayment button code sample`**
...
- createOrder Implement the createOrder function to allow the JavaScript SDK to submit buyer information and set up the transaction on the click of the button.
  ...
  **Note:** Create BLIK orders in PLN currency.

  Use your server-side [Create order](/docs/api/orders/v2/#orders_create) call to set up the details of a one-time transaction including the amount, line item detail, and more.
  ...
  After order creation, orders are confirmed with the buyer-selected payment source. If the order cannot be processed with the selected payment source, the relevant errors are returned in the console.
...
The function captures the funds from the transaction and shows a message to the buyer to let them know the transaction is successful. The method is called after the buyer approves the transaction on paypal.com.

There’s no need to provide the headers and body. Since this is a client-side call, PayPal calls the [Orders API](/docs/api/orders/v2/#orders_capture) on your behalf.
...
See a sample BLIK integration in the [PayPal GitHub repository](https://github.com/paypal-examples/blik) .
