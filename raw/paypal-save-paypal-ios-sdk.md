...
slug: /docs/checkout/save-payment-methods/during-purchase/ios-sdk/paypal/
createTime: "2024-12-18T09:53:46.358Z"
updateTime: "2025-03-06T16:07:57.591Z"
---
...
1PayPalButton.Representable(){2// Create order server-side (see next step)3}## 3. Create an order
...
1funcstartPayPalVaultDuringPurchase(){2letorder=myCreateOrderFunc(amount:10.00,vaultPaymentMethodOnSuccess:true)3letconfig=CoreConfig(clientID:"CLIENT*ID",environment:.live)4letpayPalClient=PayPalWebCheckoutClient(config:config)5payPalClient.delegate=self6letpayPalRequest=PayPalWebCheckoutRequest(orderID:order.id,fundingSource:.paypal)7payPalWebCheckoutClient.start(request:payPalRequest)8}910funcpayPal(11* payPalClient:PayPalWebCheckoutClient,12didFinishWithResult result:PayPalWebCheckoutResult13){14// After the payer approves, authorize or capture the transaction on your server (see next step)15}1617funcpayPal(18* payPalClient:PayPalWebCheckoutClient,19didFinishWithError error:CoreSDKError20){21// Handle error22}2324funcpayPalDidCancel(* payPalClient:PayPalWebCheckoutClient){25// PayPal Checkout cancelled26}## 5. Authorize or capture order
