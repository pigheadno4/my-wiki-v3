<!-- Source URL: https://docs.stripe.com/payments/scalapay/accept-a-payment -->
<!-- Fetched: 2026-05-06 -->

# Accept a payment with Scalapay

Learn how to set up your integration with Scalapay.

# Checkout

> This is a Checkout for when payment-ui is checkout. View the full page at https://docs.stripe.com/payments/scalapay/accept-a-payment?payment-ui=checkout.
...
**Supported business locations**: IT, FR, ES, DE, NL, BE, IE, FI
...
1. Add `scalapay` to the list of `payment_method_types`.
...
const session = await stripe.checkout.sessions.create({
  ...
  line_items: [
    {
      price_data: {
        // To accept `scalapay`, all line items must have currency:eur, czk, dkk, nok, pln, ron, sek, usd, chf, gbp, sgd, aud, cadcurrency: 'eur',
        product_data: {
          name: "T-shirt",
        },
        unit_amount: 1000,
...
![Scalapay payment method visible in Stripe Checkout](assets/stripe-scalapay-checkout-visible.png)

## See also

- [More about Scalapay](https://docs.stripe.com/payments/scalapay.md)
...
> This is a Checkout Sessions API for when payment-ui is elements and api-integration is checkout. View the full page at https://docs.stripe.com/payments/scalapay/accept-a-payment?payment-ui=elements&api-integration=checkout.
...
- Directly before [submitting the payment](https://docs.stripe.com/payments/scalapay/accept-a-payment.md#submit-payment). You can also call `updateEmail` to validate earlier, such as on input blur.
...
In live mode, tapping **Pay** redirects you to the Scalapay website—you don’t have the option to approve or decline the payment with Scalapay.

# Payment Intents API

> This is a Payment Intents API for when payment-ui is elements and api-integration is paymentintents. View the full page at https://docs.stripe.com/payments/scalapay/accept-a-payment?payment-ui=elements&api-integration=paymentintents.
...
To indicate that you want separate authorization and capture, set [capture_method](https://docs.stripe.com/api/payment_intents/create.md#create_payment_intent-capture_method) to `manual` when creating the PaymentIntent. This parameter instructs Stripe to only authorize the amount on the customer’s Scalapay account.
...
> This is a Direct API for when payment-ui is direct-api. View the full page at https://docs.stripe.com/payments/scalapay/accept-a-payment?payment-ui=direct-api.
...
Use the [client secret](https://docs.stripe.com/api/payment_intents/object.md#payment_intent_object-client_secret) of the `PaymentIntent` and call `stripe.confirmPayment` to handle the Scalapay redirect. Add a `return_url` to determine where Stripe redirects the customer after they complete the payment.
