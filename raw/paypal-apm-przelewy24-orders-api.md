---
title: Integrate Przelewy24 using the Orders API
slug: /docs/checkout/apm/przelewy24/orders-api/
createTime: "2024-12-09T06:44:15.301Z"
updateTime: "2025-05-06T02:38:19.395Z"
---

# Integrate Przelewy24 using the Orders API

Render payment buttons and process payments with the Orders API.

## Know before you code

- Request approval to enable Przelewy24 by visiting these sandbox and live links: - Sandbox: [https://www.sandbox.paypal.com/bizsignup/entry?product=przelewy24&capabilities=PRZELEWY24&country.x=&lt;merchant's country=""&gt;](https://www.sandbox.paypal.com/bizsignup/entry?product=przelewy24&capabilities=PRZELEWY24&country.x=)
- Live: [https://www.paypal.com/bizsignup/add-product?product=przelewy24&capabilities=PRZELEWY24&country.x=&lt;merchant's country=""&gt;](https://www.paypal.com/bizsignup/add-product?product=przelewy24&capabilities=PRZELEWY24&country.x=)

- Partners: Be sure to onboard your merchants upfront, [before they accept payments](https://developer.paypal.com/docs/multiparty/seller-onboarding/before-payment/) . Onboarding after making payments, specifically Progressive Onboarding, isn't supported for alternative payment methods.

**Note:**The integration steps for implementing alternative payment methods are similar. If you've integrated another alternative payment method before, you can reuse that code with adjustments for this payment method.- Make sure you're [subscribed to the following webhook events](/docs/checkout/apm/reference/subscribe-to-webhooks/) : - PAYMENT.CAPTURE.COMPLETED - Listen for this webhook to get notified about a successful order capture.
...
- When processing Przelewy24 payments, you don't need to capture payment for the order.

## Offer Przelewy24 on the checkout page

You'll need to create the user interface to offer Przelewy24 and collect the buyer's information, then you'll use the API calls described in the remainder of this topic to:

- Create the order with buyer's full_name , email_address , country_code and Przelewy24 as the payment source.
- Redirect the buyer to Przelewy24.

Refer to [Payment method icons](/docs/checkout/apm/reference/method-icons/) for icons you can use and download locations.

Use the buyer information you captured from your user interface to create an order with Przelewy24 as the payment source.
...
```javascript
{
    ...
    "links": [
        {
            "href": "https://api-m.sandbox.paypal.com/v2/checkout/orders/9ED24655BH4564627",
            "rel": "self",
            "method": "GET"
        },
        {
            "href": "https://sandbox.paypal.com/payment/p24?token=9ED24655BH4564627",
            "rel": "payer-action",
            "method": "GET"
        }
    ]
...
- payment_source - Specify p24 as the payment method and include the country code and account holder's full name and email address.
...
In your user interface, attach the payer-action redirect URL returned in theCreate Orderresponse to the Przelewy24 payment button. This sends the buyer to their bank to approve the purchase. Once the buyer approves the purchase, the payment is automatically captured.

In the sample, the redirect URL is: "https://sandbox.paypal.com/payment/p24?token=9ED24655BH4564627" .
...
```javascript
{
  ...
  "event_version": "1.0",
  "create_time": "2022-04-09T10:23:24Z",
  ...
  "resource": {
    ...
    "supplementary_data": {
      "related_ids": {
        "order_id": "9ED24655BH4564627"
      }
    },
    "create_time": "2022-04-09T10:23:24Z",
    "update_time": "2022-04-09T10:23:24Z",
    "final_capture": true,
...
**Notes:**- order ID from [Step 2](#create-an-order) should match resource.supplementary_data.related_ids.order_id parameter in the webhook payload.

- When an order is declined, the CHECKOUT.ORDER.DECLINED webhook passes the error code and message in themost_recent_errorparameter of thepurchase_unitobject.
...
```javascript
curl -v -X GET https://api-m.sandbox.paypal.com/v2/checkout/orders/9ED24655BH4564627 \
 -H "Content-Type: application/json" \
 -H "Authorization: Bearer <Access-Token>"
```
