<!-- Source: Stripe Checkout — Customize redirect behavior -->
<!-- Fetched: 2026-04-20 -->
...
app.get("/order/success", async (req, res) => {
  ...
  const customer_account = await stripe.v2.core.accounts.retrieve(
...
app.get("/session_status", async (req, res) => {
  const session = await stripe.checkout.sessions.retrieve(req.query.session_id);
  const customer_account = await client.v2.core.accounts.retrieve(
    session.customer_account,
  );
