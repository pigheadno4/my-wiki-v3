<!-- Source URL: https://docs.stripe.com/payments/revolut-pay/set-up-future-payments -->
<!-- Fetched: 2026-05-07 -->
...
> This is a Checkout for when payment-ui is checkout. View the full page at https://docs.stripe.com/payments/revolut-pay/set-up-future-payments?payment-ui=checkout.

This guide covers how to save a Revolut Pay payment details using [Checkout](https://docs.stripe.com/payments/checkout.md), our fully hosted checkout page.

Learn how to [set up a subscription with Revolut Pay](https://docs.stripe.com/billing/subscriptions/revolut-pay.md) to create recurring payments after saving a payment method in Checkout.
...
To reuse a Revolut Pay payment method for future payments, attach it to an object that represents your customer.
...
Your customer must authorize you to use their Revolut account for future payments through Stripe Checkout. This allows you to accept Revolut payments. Add a checkout button to your website that calls a server-side endpoint to create a [Checkout Session](https://docs.stripe.com/api/checkout/sessions.md).
...
> This is a Direct API for when payment-ui is direct-api. View the full page at https://docs.stripe.com/payments/revolut-pay/set-up-future-payments?payment-ui=direct-api.
...
Any on-session Revolut payment, where you’re actively in your checkout flow, redirects you to the Revolut app for confirmation, even if you’re using a saved payment method.
...
To save a Revolut Pay payment method for future payments, attach it to an object that represents your customer.
...
Next, you save Revolut Pay on the client with [Stripe.js](https://docs.stripe.com/payments/elements.md).
...
## Optional: Handle the Revolut Pay redirect manually
...
1. Create and *confirm* (Confirming an intent indicates that the customer intends to use the current or provided payment method. Upon confirmation, the intent attempts to initiate the portions of the flow that have real-world side effects) a PaymentIntent of type `revolut_pay`. By specifying `payment_method_data`, a PaymentMethod is created and immediately used with the PaymentIntent.
...
- Add `revolut_pay` to the [payment_method_types](https://docs.stripe.com/api/payment_intents/create.md#create_payment_intent-payment_method_types) array.
...
> This is a iOS for when payment-ui is mobile and platform is ios. View the full page at https://docs.stripe.com/payments/revolut-pay/set-up-future-payments?payment-ui=mobile&platform=ios.
...
```swift
// Revolut Pay does not require additional parameters so we only need to pass the initialized
...
Retrieve the client secret from the SetupIntent you created and call [STPPaymentHandler confirmSetupIntent](<https://stripe.dev/stripe-ios/stripe-payments/Classes/STPPaymentHandler.html#/c:objc(cs)STPPaymentHandler(im)confirmSetupIntent:withAuthenticationContext:completion:>). This presents a webview where the customer can complete the payment in Revolut Pay. Afterwards, the completion block is called with the result of the payment.
...
To deactivate a reusable payment method, your server can call [detach PaymentMethod](https://docs.stripe.com/api/payment_methods/detach.md) API. Stripe sends both a [mandate.updated](https://docs.stripe.com/api/events/types.md#event_types-mandate.updated) event and a [payment_method.detached](https://docs.stripe.com/api/events/types.md#event_types-payment_method.detached) event. You can subscribe to [webhook events](https://docs.stripe.com/webhooks.md) to get notifications.

# Android

> This is a Android for when payment-ui is mobile and platform is android. View the full page at https://docs.stripe.com/payments/revolut-pay/set-up-future-payments?payment-ui=mobile&platform=android.
...
Create a `SetupIntent` with the customer’s ID and add `revolut_pay` to the [payment_method_types](https://docs.stripe.com/api/setup_intents/create.md#create_setup_intent-payment_method_types) array. Set [usage](https://docs.stripe.com/api/setup_intents/create.md#create_setup_intent-usage) to `off_session` or `on_session`. The `SetupIntent` tracks the steps of the setup process.
...
Retrieve the client secret from the SetupIntent you created and call [PaymentLauncher confirm](https://stripe.dev/stripe-android/payments-core/com.stripe.android.payments.paymentlauncher/-payment-launcher/confirm.html). This presents a webview where the customer can complete the setup with Revolut Pay. Upon completion, the provided `PaymentResultCallback` is called with the result of the payment.
