---
title: Integrate Bancontact using the Orders API
slug: /docs/checkout/apm/bancontact/orders-api/
createTime: "2024-12-09T13:00:13.005Z"
updateTime: "2025-03-18T23:47:50.816Z"
---

# Integrate Bancontact using the Orders API

Render payment buttons and process payments with the Orders API.

## Know before you code

- Request approval to enable Bancontact by visiting the following sandbox and live links: - Sandbox: [https://www.sandbox.paypal.com/bizsignup/entry?product=bancontact&capabilities=BANCONTACT&country.x=&lt;merchant's country&gt;](https://www.sandbox.paypal.com/bizsignup/entry?product=bancontact&capabilities=BANCONTACT&country.x=)
- Live: [https://www.paypal.com/bizsignup/add-product?product=bancontact&capabilities=BANCONTACT&country.x=&lt;merchant's country&gt;](https://www.paypal.com/bizsignup/add-product?product=bancontact&capabilities=BANCONTACT&country.x=&_ga=1.98292703.1793812298.1735285121)
...
- Make sure you are [subscribed to the following webhook events](/docs/checkout/apm/reference/subscribe-to-webhooks/) : - PAYMENT.CAPTURE.COMPLETED - Listen for this webhook to get notified about a successful order capture.
...
- When processing Bancontact payments, you don't need to capture payment for the order.

## Offer Bancontact on the checkout page

You'll need to create the user interface to offer Bancontact and collect the buyer's information, then you'll use the API calls described in the remainder of this topic to:

- Create the order with Bancontact as the payment source and buyer's full_name and country_code
- Redirect the buyer to Bancontact

Refer to [Payment method icons](/docs/checkout/apm/reference/method-icons/) for icons you can use and download locations.

Use the buyer information you captured from your user interface to create an order with Bancontact as the payment source.
...
```javascript
{
    ...
    "links": [
        {
            "href": "https://api-m.sandbox.paypal.com/v2/checkout/orders/84745544P6340640G",
            "rel": "self",
            "method": "GET"
        },
        {
            "href": "https://www.sandbox.paypal.com/payment/bancontact?token=84745544P6340640G",
            "rel": "payer-action",
            "method": "GET"
        }
    ]
...
- payment_source - Specify bancontact as the payment method and include the country code and account holder's full name.
...
In your user interface, attach the payer-action redirect URL returned in the Create Order response to the Bancontact payment button. This sends the buyer to their bank to approve the purchase. Once the buyer approves the purchase, the payment is automatically captured.

In the sample, the redirect URL is: "https://sandbox.paypal.com/payment/bancontact?token=84745544P6340640G" .
...
**Notes:**- order ID from [Step 2](#create-an-order) should match resource.supplementary_data.related_ids.order_id parameter in the payload of PAYMENT.CAPTURE.COMPLETED and PAYMENT.CAPTURE.DENIED webhooks.

- When an order is declined, CHECKOUT.ORDER.DECLINED webhook passes the error code and message in the most_recent_error parameter of the purchase_unit object.
...
```javascript
curl -v -X GET https://api-m.sandbox.paypal.com/v2/checkout/orders/84745544P6340640G \
 -H "Content-Type: application/json" \
 -H "Authorization: Bearer <Access-Token>"
```
