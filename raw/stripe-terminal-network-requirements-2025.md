<!-- Source: Stripe Terminal — Terminal network requirements -->
<!-- Fetched: 2026-04-24 -->
...
1. Click the site settings icon in the address bar (the “tune” icon next to the URL).
1. Select **Site settings**.
1. Find **Local network** and set it to **Allow**.
1. Refresh the page.
...
1. Click the **Data** volume in the left pane, then click **Mount**.

1. Quit Disk Utilities.

1. Open a Terminal window by clicking **Utilities** > **Terminal**.

1. Run the following command to remove the local network permission database files:
...
1. Reboot your Mac by running `reboot`.

1. After logging in, open **System Settings** > **Privacy & Security** > **Local Network** and confirm there are no entries.

1. Open Chrome and attempt to retry the operation that was previously failing.
...
1. Click **Local Network**.
1. Confirm that your browser is listed with the toggle enabled.
