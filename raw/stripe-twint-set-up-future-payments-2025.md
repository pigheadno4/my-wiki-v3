<!-- Source URL: https://docs.stripe.com/payments/twint/set-up-future-payments -->
<!-- Fetched: 2026-05-03 -->

# Set up future TWINT payments

Learn how to save TWINT details and charge your customers later.

# Checkout

> This is a Checkout for when payment-ui is checkout. View the full page at https://docs.stripe.com/payments/twint/set-up-future-payments?payment-ui=checkout.

You can use [Stripe-hosted Checkout](https://docs.stripe.com/checkout/quickstart.md) to collect TWINT payment method details in advance, and determine the final amount or payment date later. Use it to:

- Save payment methods to streamline future purchases
- Collect surcharges after fulfilling a service
- Start a free trial for a subscription

Alternatively, you can [save payment details during a TWINT payment](https://docs.stripe.com/payments/twint/save-during-payment.md) and charge the saved payment method immediately.
...
Create a Checkout Session in [setup mode](https://docs.stripe.com/api/checkout/sessions/create.md#create_checkout_session-mode) and add `twint` to the list of [payment_method_types](https://docs.stripe.com/api/checkout/sessions/create.md#create_checkout_session-payment_method_types). Additionally, specify the customer’s ID. After creating the Checkout Session, redirect your customer to the [URL](https://docs.stripe.com/api/checkout/sessions/object.md#checkout_session_object-url) returned in the response.
...
When your customer provides their payment method details, Stripe redirects them to the `success_url`, a page on your website that informs them that their payment method was saved successfully. Make the Session ID available on your success page by including the `{CHECKOUT_SESSION_ID}` template variable in the `success_url` as in the above example.
...
Note the SetupIntent created during the Checkout Session. A [SetupIntent](https://docs.stripe.com/payments/setup-intents.md) is an object used to set up the customer TWINT account information for future payments. The returned object contains the `payment_method` ID.
...
When a customer successfully authorizes the mandate, Stripe emits a [setup_intent.succeeded](https://docs.stripe.com/api/events/types.md#event_types-setup_intent.succeeded) *webhook* (A webhook is a real-time push notification sent to your application as a JSON payload through HTTPS requests) event and the SetupIntent’s status transitions to `succeeded`. Store the resulting [payment_method](https://docs.stripe.com/api/setup_intents/object.md#setup_intent_object-payment_method) ID to make payments using the saved payment method later.

If a customer doesn’t successfully authorize the mandate, Stripe emits a [setup_intent.setup_failed](https://docs.stripe.com/api/events/types.md#event_types-setup_intent.setup_failed) webhook event and the SetupIntent’s status updates to `requires_payment_method`.
...
> This is a Elements for when payment-ui is elements. View the full page at https://docs.stripe.com/payments/twint/set-up-future-payments?payment-ui=elements.

You can use the [Payment Element](https://docs.stripe.com/payments/payment-element.md), a prebuilt UI component, to collect TWINT payment method details in advance, and determine the final amount or payment date later. Use it to:
...
Create a `SetupIntent` on your server with `twint` added to the list of [payment_method_types](https://docs.stripe.com/api/setup_intents/create.md#create_setup_intent-payment_method_types), and specify the customer’s ID.
...
## Collect payment method details and submit the setup request [Client-side]
...
Use [stripe.confirmSetup](https://docs.stripe.com/js/setup_intents/confirm_setup) to complete the setup using details from the Payment Element. This will redirect the customer from your page to the TWINT-hosted setup page. Specify a [return_url](https://docs.stripe.com/js/setup_intents/confirm_setup#confirm_setup_intent-options-confirmParams-return_url) to indicate where Stripe must redirect the user after they complete the setup.

> `stripe.confirmSetup` might take time to complete while waiting for customers to complete the setup. During that time, you can disable your form from being resubmitted, and display a waiting indicator like a spinner. If you receive an error result, make sure to show the error to the customer, re-enable the form, and hide the waiting indicator.
...
form.addEventListener("submit", async (event) => {
  ...
  if (error) {
    ...
    // confirming the SetupIntent. Show the error to your customer
    ...
    switch (setupIntent.status) {
      case "succeeded":
        message.innerText = "Success! Authorization received.";
        break;

      case "requires_payment_method":
        message.innerText = "Setup failed. Please try another payment method.";
...
const SetupForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (error) {
      ...
      switch (setupIntent.status) {
        case "succeeded":
          setMessage("Success! Authorization received.");
          break;
...
> This is a Direct API for when payment-ui is direct-api. View the full page at https://docs.stripe.com/payments/twint/set-up-future-payments?payment-ui=direct-api.

You can use the [Setup Intents API](https://docs.stripe.com/payments/setup-intents.md) to collect TWINT payment method details in advance, and determine the final amount or payment date later. Use it to:
...
Use [Stripe.js](https://docs.stripe.com/js.md) to confirm the SetupIntent. Stripe.js is our foundational JavaScript library for building payment flows. It automatically manages complexities, such as redirects, and allows you to integrate with other payment methods.
...
To confirm the setup on the client side, pass the client secret of the SetupIntent. The client secret is different from your API keys that authenticate Stripe API requests. Handle it carefully because it can complete the setup. Don’t log it, embed it in URLs, or expose it to anyone but the customer.

Use [stripe.confirmTwintSetup](https://docs.stripe.com/js/setup_intents/confirm_twint_setup) to handle the redirect from your page to the TWINT-hosted setup page. Specify a [return_url](https://docs.stripe.com/js/setup_intents/confirm_setup#confirm_setup_intent-options-confirmParams-return_url) to indicate where Stripe can redirect the user after they complete the setup.

> `stripe.confirmTwintSetup` might take time to complete while waiting for customers to complete the setup. During that time, you can disable your form from being resubmitted, and display a waiting indicator like a spinner. If you receive an error result, make sure to show the error to the customer, re-enable the form, and hide the waiting indicator.

```javascript
// Redirects away from the client
const { error, setupIntent } = await stripe.confirmTwintSetup(
...
1. *Confirm* (Confirming an intent indicates that the customer intends to use the current or provided payment method. Upon confirmation, the intent attempts to initiate the portions of the flow that have real-world side effects) the `SetupIntent` at creation time by including `confirm: true`.
1. Provide [mandate_data](https://docs.stripe.com/api/setup_intents/create.md#create_setup_intent-mandate_data).
1. When the customer completes the setup, Stripe redirects them to the URL you specified in the [return_url](https://docs.stripe.com/api/setup_intents/create.md#create_setup_intent-return_url) parameter.
...
The `SetupIntent` has a `requires_action` status and a `next_action` of `redirect_to_url` type. To authorize the use of their TWINT account for future payments, redirect the customer to the TWINT-hosted setup page provided in the `next_action.redirect_to_url.url` property. After the customer completes the process, Stripe redirects them to the `return_url` you specify.
...
Test your TWINT integration with your test API keys by viewing the SetupIntent’s redirect page. If you authorize the mandate on the redirect page, the SetupIntent transitions from `requires_action` to `succeeded`. If you decline the mandate on the redirect page, the SetupIntent transitions from `requires_action` to `requires_payment_method`.

To reproduce common live mode payment scenarios when charging a saved TWINT payment method, set [payment_method_data.billing_details.email](https://docs.stripe.com/api/setup_intents/confirm.md#confirm_setup_intent-payment_method_data-billing_details-email) to the following values when you confirm a SetupIntent. If you don’t specify `payment_method_data.billing_details.email`, or if the specified email doesn’t match any of the following values, the payment succeeds by default and the [payment_intent.succeeded](https://docs.stripe.com/api/events/types.md#event_types-payment_intent.succeeded) webhook arrives within several seconds.
