<!-- Source URL: https://docs.adyen.com/online-payments/upgrade-your-integration/upgrade-to-web-v6/from-v3.md -->
<!-- Fetched: 2026-09-15 -->
<!-- Discovery: llms.txt -->

---
title: "Upgrade to Adyen Web v6"
description: "Make the changes required to upgrade from Web Drop-in/Components v3.x.x to v6.x.x."
url: "https://docs.adyen.com/online-payments/upgrade-your-integration/upgrade-to-web-v6/from-v3"
source_url: "https://docs.adyen.com/online-payments/upgrade-your-integration/upgrade-to-web-v6/from-v3.md"
canonical: "https://docs.adyen.com/online-payments/upgrade-your-integration/upgrade-to-web-v6/from-v3"
...
Make the changes required to upgrade from Web Drop-in/Components v3.x.x to v6.x.x.
...
To upgrade from [Web Drop-in/Components](https://github.com/Adyen/adyen-web) v3.x.x to v6.0.0, complete the following steps.

If you are upgrading from a later version, you need to follow a different upgrade guide:

* [From **v4.x.x** to v6.x.x](/online-payments/upgrade-your-integration/upgrade-to-web-v6/from-v4).
* [From **v5.x.x** to v6.x.x](/online-payments/upgrade-your-integration/upgrade-to-web-v6).
...
* [Import Drop-in with all payment methods](#npm-auto): this method resembles what you are used to before v6. You do not need to import individual payment methods, at the cost of a slower loading time.
* [Import Drop-in with individual payment methods](#npm-reduced-bundle-size): this method uses tree shaking to let you import only the payment methods you use, speeding up loading time. However, adding new payment methods requires developer resources.
...
However, this results in a larger bundle size and slower loading times, as all module exports are imported client-side. If a smaller bundle size and faster page load times is a priority to you, consider importing [Drop-in with individual payment methods](#npm-reduced-bundle-size).
...
3. Assign them to the `paymentMethodComponents` property [within the Drop-in configuration](#update-creating-an-instance).
...
From v6 onwards, the `AdyenCheckout()` function is and property of `window.AdyenWeb`, alongside all the payment method-specific Components:
...
* You can no longer set `paymentMethodsConfiguration` during the `AdyenCheckout` instantiation. Set it directly in the [Drop-in configuration](/online-payments/build-your-integration/sessions-flow?platform=Web\&version=6.0.0\&integration=Drop-in#step-4-configure-drop-in) instead.
...
* The `originKey` is no longer supported. Migrate to [client key](/development-resources/client-side-authentication/migrate-from-origin-key-to-client-key/).

### Component specific configuration

For certain components, some deprecated props were removed. Make sure that your integration uses the remaining props.
...
| `ariaLabels` | No longer supported. Use [translation fields instead](#aria-labels-in-transaltion-fields). |

##### Configure aria labels through translation fields

Configuring `aria-label` attributes using the `ariaLabels` field inside the component is no longer supported. In v6, `Aria-label` attributes are read from [translation fields](/online-payments/build-your-integration/sessions-flow?platform=Web\&integration=Components\&version=6.0.0#localization).

You can no longer configure the `label` property, the label is added automatically, aligning with the visual label for the field. This is to follow [accessibility](/online-payments/accessibility/) best practices.
...
| Removed prop                        | Prop to use instead                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `amount`                            | No longer needs to be configured. The Drop-in/Component takes this from the `amount.value`, passed to `AdyenCheckout`.                                                                                                                                                                                                                                                                                                                                                                        |
| `currencyCode`                      | No longer needs to be configured. The Drop-in/Component takes this from the `amount.currency`, passed to `AdyenCheckout`.                                                                                                                                                                                                                                                                                                                                                                     |
| `configuration.merchantDisplayName` | No longer needs to be configured. This value is returned when you make a [/sessions](https://docs.adyen.com/api-explorer/Checkout/latest/post/sessions) request for the [Sessions flow](/online-payments/build-your-integration/sessions-flow?platform=Web\&version=6.0.0), or [/paymentMethods](https://docs.adyen.com/api-explorer/Checkout/latest/post/paymentMethods) request for the [Advanced flow](/online-payments/build-your-integration/advanced-flow?platform=Web\&version=6.0.0). |
| `configuration.merchantIdentifier`  | No longer needs to be configured. This value is returned when you make a [/sessions](https://docs.adyen.com/api-explorer/Checkout/latest/post/sessions) request for the [Sessions flow](/online-payments/build-your-integration/sessions-flow?platform=Web\&version=6.0.0), or [/paymentMethods](https://docs.adyen.com/api-explorer/Checkout/latest/post/paymentMethods) request for the [Advanced flow](/online-payments/build-your-integration/advanced-flow?platform=Web\&version=6.0.0). |
...
| `merchantIdentifier` | No longer supported. This value is returned when you make a [/sessions](https://docs.adyen.com/api-explorer/Checkout/latest/post/sessions) request for the [Sessions flow](/online-payments/build-your-integration/sessions-flow?platform=Web\&version=6.0.0), or [/paymentMethods](https://docs.adyen.com/api-explorer/Checkout/latest/post/paymentMethods) request for the [Advanced flow](/online-payments/build-your-integration/advanced-flow?platform=Web\&version=6.0.0). |
...
This applies to the following payment methods: MOLPay, EPS, OpenBanking UK, Online Banking India, and Indian wallet or online payments through Billdesk or PayU.
