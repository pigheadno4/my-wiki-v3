<!-- Source URL: https://docs.adyen.com/payment-methods/cards/korean-cards/api-only.md -->
<!-- Fetched: 2026-09-15 -->
<!-- Discovery: llms.txt -->

---
title: "Korean Card Payments via redirect for API only"
description: "Add Korean Card Payments via redirect to your API-only integration."
url: "https://docs.adyen.com/payment-methods/cards/korean-cards/api-only"
source_url: "https://docs.adyen.com/payment-methods/cards/korean-cards/api-only.md"
canonical: "https://docs.adyen.com/payment-methods/cards/korean-cards/api-only"
last_modified: "2026-09-15T12:56:53+02:00"
language: "en"
---

# Korean Card Payments via redirect for API only

Add Korean Card Payments via redirect to your API-only integration.
...
1. The shopper selects Korean Card Payments via redirect as the payment method.
...
Include Korean Card Payments in the list of [available payment methods](#get-available-methods).

You do not need to collect any other information from the shopper in your payment form.

You can [download the logo for Korean Card Payments via redirect](/online-payments/build-your-integration/advanced-flow/?platform=Web\&integration=API%2Bonly\&version=71#downloading-logos) to use in your form.

## Get Korean Card Payments via redirect as an available payment method

When you make the [/paymentMethods](https://docs.adyen.com/api-explorer/Checkout/latest/post/paymentMethods) to [get available payment methods](/online-payments/build-your-integration/advanced-flow/?platform=Web\&integration=API%20only#get-available-payment-methods), specify the following so that Korean Card Payments via redirect is included in the response.
...
**Example response with Korean Card Payments via redirect available**

```json
{
    "paymentMethods": [
        {
            "name": "Korean Card Payments via redirect",
            "type": "kcp_creditcard"
        }
    ]
}
...
**Example payment request for Korean Card Payments via redirect**
...
PaymentDetails paymentDetails = new PaymentDetails()
  .type(PaymentDetails.TypeEnum.KCP_CREDITCARD);
...
PaymentDetails paymentDetails = new PaymentDetails
{
  Type = PaymentDetails.TypeEnum.KcpCreditcard
};
...
paymentDetails := checkout.PaymentDetails{
  Type: common.PtrString("kcp_creditcard"),
}
...
const paymentDetails: Types.checkout.PaymentDetails = {
  type: Types.checkout.PaymentDetails.TypeEnum.KcpCreditcard
};
...
```json
{
    ...
    "action": {
        "paymentMethodType": "kcp_creditcard",
        "method": "GET",
