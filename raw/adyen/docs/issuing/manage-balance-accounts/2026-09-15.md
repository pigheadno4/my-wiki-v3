<!-- Source URL: https://docs.adyen.com/issuing/manage-balance-accounts.md -->
<!-- Fetched: 2026-09-15 -->
<!-- Discovery: llms.txt -->
...
url: "https://docs.adyen.com/issuing/manage-balance-accounts"
source_url: "https://docs.adyen.com/issuing/manage-balance-accounts.md"
canonical: "https://docs.adyen.com/issuing/manage-balance-accounts"
...
A balance account is an API resource that contains information about the funds held by your user. To manage the balance accounts use the [Configuration API](https://docs.adyen.com/api-explorer/balanceplatform/latest/overview).

This page contains instructions for common balance account operations. These operations are part of a larger process which includes [verification](/issuing/verification-overview) and [onboarding steps](/issuing/onboard-users/onboarding-steps).
...
1. Check if the balance account has [zero balance](#view-balance-accounts). You can only close empty balance accounts, so if there are funds left, [transfer](/issuing/add-manage-funds) any remaining balance..
2. Close a balance account by [updating the balance account](#update-balance-account) and setting the `status` to **Closed**.

If an account holder discontinues their contractual relationship with your platform, you can also [permanently deactivate the account holder](/issuing/manage-account-holders#permanent-deactivation).
