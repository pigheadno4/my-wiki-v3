<!-- Source URL: https://docs.adyen.com/issuing/manage-account-holders.md -->
<!-- Fetched: 2026-09-15 -->
<!-- Discovery: llms.txt -->
...
url: "https://docs.adyen.com/issuing/manage-account-holders"
source_url: "https://docs.adyen.com/issuing/manage-account-holders.md"
canonical: "https://docs.adyen.com/issuing/manage-account-holders"
...
Account holders are the main API resource that represent users of your issuing integration. It contains the legal entities and balance accounts of the user. The object also contains information about the user's [capabilities](/issuing/verification-overview/capabilities). To manage account holders, use the [Configuration API](https://docs.adyen.com/api-explorer/balanceplatform/latest/overview).

This page contains instructions for common account holder operations. These operations are part of a larger process which includes [verification](/issuing/verification-overview) and [onboarding steps](/issuing/onboard-users/onboarding-steps).
...
You can create account holders using the Configuration API. To create an account holder, you need to [create a legal entity](/issuing/manage-legal-entities#create-legal-entity) first.

1. Make a POST [/accountHolders](https://docs.adyen.com/api-explorer/balanceplatform/latest/post/accountHolders) request using a [legal entity ID](/issuing/manage-legal-entities#create-legal-entity). For sole proprietorships, this is the individual legal entity ID of the owner.
...
2. In the response, note the unique `id` of the new `accountHolder` resource as well as a capabilities object with the default [capabilities](/issuing/verification-overview/capabilities/). Save the response because you need it to:

   * Match the incoming webhooks using the `id`.
   * [Create a balance account for the account holder](/issuing/manage-balance-accounts#create-a-balance-account).
...
* Their capabilities and [verification deadlines](/issuing/verification-overview/#verification-deadlines)

### Filter and search

On the **Account holder list** page, you can find specific account holders by using the filter and search features.

Set a filter by using the buttons on the filter bar, located on top of the account holder table. For example, you can filter account holders based on their [capabilities](/issuing/verification-overview/capabilities) and the capability statuses.
...
The [liable account holder](/issuing/account-structure-resources) is the account holder related to your company's [legal entity](/issuing/verification-requirements/#legal-entity-type). To view the details of the liable account holder, on the **Account holder list** page, select **View liable account holder**.
...
2. Receive the details about the account holder including their [capabilities](/issuing/verification-overview/capabilities) in the API response.
...
* GET [/balancePlatforms/{id}/accountHolders](https://docs.adyen.com/api-explorer/balanceplatform/latest/get/balancePlatforms/\(id\)/accountHolders): Get a list of the account holders in your issuing. This endpoint returns a paginated list of account holders.
...
If you want to temporarily disable an account holder on your issuing, change their [status](https://docs.adyen.com/api-explorer/balanceplatform/latest/get/accountHolders/\(id\)#responses-200-status) to `suspended`. Changing the status to suspended disables payout capabilities. Related balance accounts and payment instruments remain active when you use the API to suspend an account holder. To learn more about how different statuses affect your account holders, see [Account Holder Status](/issuing/account-holder-status/).
...
2. In the response, note the updated `status` parameter and `capabilities` object with disabled capabilities. The exact capabilities Adyen returns depends on which [User capabilities](/issuing/verification-overview/capabilities) you have configured.
...
3. Listen to [balancePlatform.accountHolder.updated](https://docs.adyen.com/api-explorer/balanceplatform-webhooks/latest/post/balancePlatform.accountHolder.updated) webhooks. Adyen sends a webhook whenever the status of an account holder changes which includes both API updates and [Adyen initiated updates](/issuing/account-holder-status#adyen-initiated-status-updates).

## Reactivate an account holder

If you want to reactivate an account holder on your issuing, change their [status](https://docs.adyen.com/api-explorer/balanceplatform/latest/get/accountHolders/\(id\)#responses-200-status) to **active**. You can only change the status to **active** if the current status is **suspended**. If Adyen suspended the account holder, you cannot reactivate them with the API. To learn more about status definitions, and Adyen initiated statuses, see [Account Holder Status](/issuing/account-holder-status/).
...
2. In the response, note the updated `status` parameter and `capabilities` object with re-enabled capabilities. The exact capabilities Adyen returns depends on which [User capabilities](/issuing/verification-overview/capabilities) you have configured.
...
When an account holder discontinues their business, you can permanently close them and their balance accounts from your issuing.
...
1. Check if they have [zero balance](/issuing/manage-balance-accounts#view-balance-accounts) in their balance accounts. You can only close empty balance accounts, so if there are funds left, [transfer](/issuing/add-manage-funds) any remaining balance.
2. Close balance accounts by [updating each balance account](/issuing/manage-balance-accounts#update-balance-account) and setting the `status` to **Closed**. Start with those that are not the primary balance account. You can only close a primary balance account if the account holder's other balance accounts are already closed.
...
2. In the [capabilities](https://docs.adyen.com/api-explorer/balanceplatform/latest/get/accountHolders/\(id\)#responses-200-capabilities) object of the response, note the following parameters and values that are listed for each capability:
   ...
   * [problems](https://docs.adyen.com/api-explorer/balanceplatform/latest/get/accountHolders/\(id\)#responses-200-capabilities-problems) array: When this array is not empty, this means that there are errors that you need to address. Refer to [verification error codes](/issuing/kyc-verification-codes) for a list of verification errors.

## Request a capability

Some capabilities are automatically requested when you [create an account holder](/issuing/onboard-users/onboarding-steps). However, your account holders may need additional capabilities. For example, if your business expands its financial product offering. In these cases, you must request the additional capabilities.
...
1. In your PATCH [/accountHolders/{id}](https://docs.adyen.com/api-explorer/balanceplatform/latest/patch/accountHolders/\(id\)) request, set the capability that you are requesting as the key, and specify the following fields in the object:
   ...
   * [requestedLevel](https://docs.adyen.com/api-explorer/balanceplatform/latest/patch/accountHolders/\(id\)#responses-200-capabilities-requestedLevel): Set this to the desired [capability level](/issuing/verification-overview/capabilities#capability-levels).

Here is an example of how to request the `withdrawFromAtm` capability with level **medium** for an existing user. If you are requesting multiple capabilities, add another set of key-value pairs.
...
accountHolderCapability := balancePlatform.AccountHolderCapability{
  ...
  RequestedLevel: common.PtrString("medium"),
}
...
const accountHolderCapability: Types.balancePlatform.AccountHolderCapability = {
  requested: true,
  requestedLevel: Types.balancePlatform.AccountHolderCapability.RequestedLevelEnum.Medium
};
...
The [Know Your Customer](/issuing/onboard-users/) (KYC) checks are part of the verification process that Adyen performs to make sure that account holders have provided accurate information about their businesses.
...
You can use this information to help your users resolve [verification errors](/issuing/kyc-verification-codes) they experience during the onboarding process, such as expired documents or a mismatch between the legal and organization names.
