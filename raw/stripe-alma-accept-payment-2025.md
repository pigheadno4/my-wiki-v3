<!-- Source URL: https://docs.stripe.com/payments/alma/accept-a-payment -->
<!-- Fetched: 2026-05-06 -->
...
> This is a Checkout for when payment-ui is checkout. View the full page at https://docs.stripe.com/payments/alma/accept-a-payment?payment-ui=checkout.
...
This guides you through enabling Alma and shows the differences between accepting payments using dynamic payment methods and manually configuring payment methods.
...
![Alma payment method visible in Stripe Checkout](assets/stripe-alma-checkout-visible.png)

## See also

- [More about Alma](https://docs.stripe.com/payments/alma.md)
...
> This is a Elements for when payment-ui is elements. View the full page at https://docs.stripe.com/payments/alma/accept-a-payment?payment-ui=elements.

Use [Payment Elements](https://docs.stripe.com/payments/payment-element.md) to embed a custom Stripe payment form in your website or application. Payment Elements allows you to support Alma and other payment methods automatically. For advanced configurations and customizations, refer to the [Accept a Payment](https://docs.stripe.com/payments/accept-a-payment.md) integration guide.
...
To indicate that you want separate authorization and capture, set [capture_method](https://docs.stripe.com/api/payment_intents/create.md#create_payment_intent-capture_method) to `manual` when creating the PaymentIntent. This parameter instructs Stripe to only authorize the amount on the customer’s Alma account.
...
| --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `payment_intent_invalid_currency`                         | Enter the appropriate currency. Alma only supports `eur`.                                                                                                                                                                                                                    |
...
| `payment_intent_redirect_confirmation_without_return_url` | Provide a `return_url` when confirming a PaymentIntent with Alma.                                                                                                                                                                                                            |

# Direct API

> This is a Direct API for when payment-ui is direct-api. View the full page at https://docs.stripe.com/payments/alma/accept-a-payment?payment-ui=direct-api.
...
Use the [client secret](https://docs.stripe.com/api/payment_intents/object.md#payment_intent_object-client_secret) of the `PaymentIntent` and call `stripe.confirmPayment` to handle the Alma redirect. Add a `return_url` to determine where Stripe redirects the customer after they complete the payment.
...
> This is a iOS for when payment-ui is mobile and platform is ios. View the full page at https://docs.stripe.com/payments/alma/accept-a-payment?payment-ui=mobile&platform=ios.

Use the [Mobile Payment Element](https://docs.stripe.com/payments/accept-a-payment.md?=ios), an embeddable payment form, to add Alma and other payment methods to your integration with the least amount of effort.
...
Create a PaymentIntent on your server with an amount and currency. Before creating the PaymentIntent, make sure to turn **Alma** on in the [payment methods settings](https://dashboard.stripe.com/settings/payment_methods) page.
...
When a customer taps to pay with Alma, confirm the `PaymentIntent` to complete the payment. Configure an `STPPaymentIntentParams` object with the `PaymentIntent` [client secret](https://docs.stripe.com/api/payment_intents/object.md#payment_intent_object-client_secret).
...
The iOS SDK presents a webview in your app to complete the Alma payment. When authentication is finished, the webview can automatically dismiss itself instead of having your customer close it. To enable this behavior, configure a custom URL scheme or universal link and set up your app delegate to forward the URL to the SDK.
...
Complete the payment by calling `STPPaymentHandler.confirmPayment`. This presents a webview where the customer can complete the payment with Alma. After completion, Stripe calls the completion block with the result of the payment.
...
// Alma doesn't require additional parameters so we only need to pass the initialized
// STPPaymentMethodAlmaParams instance to STPPaymentMethodParams
...
> This is a Android for when payment-ui is mobile and platform is android. View the full page at https://docs.stripe.com/payments/alma/accept-a-payment?payment-ui=mobile&platform=android.

Use the [Mobile Payment Element](https://docs.stripe.com/payments/accept-a-payment.md?=android), an embeddable payment form, to add Alma and other payment methods to your integration with the least amount of effort.
...
Before creating a PaymentIntent, turn on **Alma** on your [payment methods settings](https://dashboard.stripe.com/settings/payment_methods) page in the Dashboard. Create a PaymentIntent on your server with an amount and currency.
...
If you don’t want to use the Dashboard or you want to manually specify payment methods, list them using the `payment_method_types` attribute.
...
When a customer taps to pay with Alma, confirm the `PaymentIntent` to complete the payment. Configure an `ConfirmPaymentIntentParams` object with the `PaymentIntent` [client secret](https://docs.stripe.com/api/payment_intents/object.md#payment_intent_object-client_secret).
